...
//        }
    ...
    $sql = "SELECT pg.*, pgd.*,i.invoice_no, DATE_FORMAT(pg.invoice_date, '%d %b %Y') AS invoice_date, DATE_FORMAT(pg.input_date, '%d %b %Y') AS input_date, 
    DATE_FORMAT(pg.request_date, '%d %b %Y') AS request_date, u.user_name, s.stockpile_name, gv.general_vendor_name,
        ...
        CASE WHEN i.invoice_id IS NOT NULL THEN i.invoice_status ELSE inv.invoice_status END AS invStatus, 
        CASE WHEN i.invoice_id IS NOT NULL THEN i.payment_status ELSE inv.`payment_status` END AS payment_status, 
        i.invoice_id as invId,
        CASE WHEN pg.invoice_id IS NOT NULL THEN pg.invoice_id ELSE pg.invoice_old_id END AS pgInvoice
        ...
        LEFT JOIN invoice inv
		    ON inv.invoice_id = pg.invoice_old_id
        ...
        WHERE pg.company_id = {$_SESSION['companyId']} AND pg.status_pengajuan != 4
...
//            alert($('#addNew').attr('href'));
    ...
    //            alert($('#addNew').attr('href'));
                $('#addJurnalModalForm').load('forms/view-journal-invGeneral.php', {invId: menu[2], status: menu[3], }, iAmACallbackFunction2);	//and hide the rotating gif
        ...
        while ($rowData = $resultData->fetch_object()) {
            ...
            <tr>
                <td>
                    ...
                    <!-- <a href="#" id="approve|pg|<?php echo $rowData->pengajuan_general_id; ?>" role="button" title="Preview"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>  -->

                    <?php if($rowData->status_pengajuan == 3 && $rowData->invStatus == 1){ ?>
                        <a href="#" id="approve|pg|<?php echo $rowData->pengajuan_general_id; ?>" role="button" title="Preview"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a> 
                        <?php } else if (($rowData->status_pengajuan == 0) || ($rowData->status_pengajuan == 1 && $rowData->invStatus == 2) || ($rowData->status_pengajuan == 3 && $rowData->invStatus == 2)) { ?>
                            <a href="#" id="approve|pg|<?php echo $rowData->pengajuan_general_id; ?>" role="button" title="Approve Pengajuan"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;"/></a>
                        <?php }else if(($rowData->status_pengajuan == 1 && ($rowData->invStatus == 1 || $rowData->invStatus == 4)) || ($rowData->status_pengajuan == 3 && ($rowData->invStatus == 2 || $rowData->invStatus == 4)) ){?>
                            <a href="#" id="approve|pg|<?php echo $rowData->pengajuan_general_id; ?>" role="button" title="Edit Invoice"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;"/></a>
                        <?php } ?>
                    </div>
                </td>
                <td><a href="#" id="jurnal|journal-invoice|<?php echo $rowData->pgInvoice; ?>|<?php echo $rowData->invStatus ?>" role="button"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /> View Journal</a></td>
                <?php
                if ($rowData->status_pengajuan == 1 && $rowData->invStatus == 1) {
                    echo "<td style='font_weight: bold; color: green'>";
                    echo "INVOICE";
                    echo "</td>";
                } else if($rowData->invStatus == 4 && $rowData->status_pengajuan == 1){
                    echo "<td style='font_weight: bold; color: red;'>";
                    echo "REJECT INVOICE";
                    echo "</td>";
                }elseif ($rowData->status_pengajuan == 3 && $rowData->invStatus == 1) {
                    echo "<td style='font_weight: bold; color: yellowgreen;'>";
                    echo "PAYMENT";
                    echo "</td>";
                } elseif (($rowData->payment_status == 2 && $rowData->invStatus == 2) || ($rowData->payment_status = 4 &&  $rowData->invStatus == 2) || ($rowData->payment_status && $rowData->invStatus = 2) ) {
                    echo "<td style='font_weight: bold; color: red;'>";
                    echo "RETURN INVOICE";
                    echo "</td>";
                } elseif ($rowData->invStatus == 4) {
                    echo "<td style='font_weight: bold; color: red;'>";
                    echo "RETURN PAYMENT";
                    echo "</td>";
                } else {
...
<div id="addJurnalModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="jurnalModalLabel" aria-hidden="true">
        <form id="jurnalForm" method="post" style="margin: 0px;">
           ...
           <input type="hidden" name="modalTransactionId" id="modalTransactionId" value="<?php echo $rowData->invoice_id; ?>" />
