...
CASE WHEN a.sa_method = 2 THEN 'Advance' ELSE 'Settlement/Reimbursement' END AS metode,g.general_vendor_name
...
LEFT JOIN general_vendor g ON g.general_vendor_id = a.id_user
...
//            alert($('#addNew').attr('href'));
        ...
        $('#exportExcel').click(function (e) {
            $("#pengajuanPerdinForm").submit(); // Submit the for
        });
        ...
        $('#contentTable a').click(function(e){
            ...
            var menu = linkId.split('|');
		 	if (menu[0] == 'print') {
                ...
                $('#dataContent').load('forms/perdin-adv_settle.php', {sa_id: menu[2]}, iAmACallbackFunction2);
                        ...
                        $.ajax({
                            ...
                            success: function(data){
                                ...
                                if(parseInt(returnVal[3])!=0)	//if no errors
                                {
                                    ...
                                    if(returnVal[1] == 'OK') {
                                        $('#dataContent').load('contents/perdin-adv_settle.php', {}, iAmACallbackFunction2);
                                    }
                                }
                            }
                        });
...
<form method="POST" action="print_p_perdin.php" id="pengajuanPerdinForm">
        ...
        <tr>
            <td><div style="text-align: center">
                            <input type="checkbox" name="checks[]"
                                   value="<?php echo $rowData->sa_id ?>"/>
                        </div>
            </td>
            <td>
                <div style="text-align: center;">
                    <a href="#" id="print|perdin-adv_settle|<?php echo $rowData->sa_id; ?>|<?php echo $rowData->sa_method; ?>" role="button" title="Print"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
					<?php if($rowData->print_status == 0) {
                    ?>
					<?php if( $rowData->sa_method == 1 && $rowData->docs == ''){?>
					<a href="#" id="edit|perdin-adv_settle|<?php echo $rowData->sa_id; ?>" role="button" title="Edit"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
					<?php
                    }
                    ?>
                    <a href="#" id="delete|perdin-adv_settle|<?php echo $rowData->sa_id; ?>" role="button" title="Delete"><img src="assets/ico/gnome-trash.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
					<?php
                    }
                    ?>
                </div>
			...
			if ($rowData->invoice_status == 1) {
                    ...
                    echo "REJECTED";
                    echo "</td>";
                } elseif ($rowData->approval_status == 2) {
                    echo "<td style='font_weight: bold; color: red;'>";
                    echo "REJECTED";
                    echo "</td>";
                } else {
			...
			?>
			<?php 
			if($rowData->docs != '' && $rowData->docs != 'NULL' && $rowData->upload_status == 1){
			echo "<td style='font_weight: bold; color: green;'>"; 
			echo "YES";
			echo "</td>";
			}else if ($rowData->upload_status == 3){
            echo "<td style='font_weight: bold; color: orange;'>"; 
            echo "REUPLOAD DOCUMENTS";
            echo "</td>";
            }else {
			echo "<td style='font_weight: bold; color: red;'>"; 
			echo "NO";
