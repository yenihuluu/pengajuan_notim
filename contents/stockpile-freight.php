...
$sql = "SELECT fc.*, CONCAT(f.freight_code, '-', v.vendor_code, ' - ', f.freight_supplier, ' - ', CONCAT(v.vendor_name, ' (', v.vendor_code, ')')) AS freight_full, 
            CONCAT(s.stockpile_code, ' - ', s.stockpile_name) AS stockpile_full, cur.currency_code, 
            case when fc.active_from is null then DATE_FORMAT(fc.entry_date, '%d %b %Y %H:%i:%s')
            else DATE_FORMAT(fc.active_from, '%d %b %Y') end as active_from
...
//                filter_reset : ".reset"
    ...
    //            alert($('#addNew').attr('href'));
                $('#addStockpileFreightModalForm').load('forms/stockpile-freight.php', {freightCostId: menu[2], stockpileId: $('input[id="generalStockpileId"]').val()});
