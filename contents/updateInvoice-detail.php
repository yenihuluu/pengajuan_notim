...
$sql = "SELECT id.*, CONCAT(a.account_no,' - ',a.account_name) AS account_name,
CASE WHEN id.type = 4 THEN 'Loading'
	WHEN id.type = 5 THEN 'Umum'
	WHEN id.type = 6 THEN 'HO'
ELSE '' END AS invoiceType, s.stockpile_name, sh.shipment_no, uo.uom_type
FROM invoice_detail id 
LEFT JOIN account a ON a.account_id = id.account_id
LEFT JOIN stockpile s ON s.`stockpile_id` = id.`stockpile_remark`
LEFT JOIN shipment sh ON sh.`shipment_id` = id.`shipment_id`
LEFT JOIN uom uo ON uo.`idUOM` = id.`idUOM` 
...
//                filter_reset : ".reset"
        ...
        $('#tabTable a').click(function(e){
            ...
            if (menu[0] == 'edit') {
                $("#modalErrorMsg").hide();
                $('#updateInvoiceDetailModal').modal('show');
    //            alert($('#addNew').attr('href'));
                $('#updateInvoiceDetailModalForm').load('forms/updateInvoice-detail.php', {invoiceDetailId: menu[2], invoiceId: $('input[id="generalInvoiceId"]').val()});
            } 
        });
            ...
            <tr>
                <td>
                    <div style="text-align: center">
                        <a href="#" id="edit|updateInvoice|<?php echo $rowData->invoice_detail_id; ?>" role="button" title="Edit"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
                ...
                <td><?php echo $rowData->invoiceType; ?></td>
