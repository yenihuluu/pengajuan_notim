...
function parseXLS($fileName, $sheet_name, $row_start, $row_end, $col_start, $col_end) {
  ...
  $rangeColumns = range("A", "W");
  for ($temp = 0; $temp < count($rangeColumns); $temp++) {
    $rangeColumns[$temp] = "A" . $rangeColumns[$temp];
  }
  $rangeColumns = array_merge(range("C", "Z"), $rangeColumns);

  for ($startRow = $row_start; $startRow <= $row_end; $startRow += $chunkSize) {
    /**  Tell the Read Filter which rows we want this iteration  * */
    $loopChunkSize = ($row_end - $startRow) >= $chunkSize ? $chunkSize : ($row_end - $startRow);
    $chunkFilter->setRows($startRow, $loopChunkSize, $rangeColumns);
    try {
      ...
      $messageError = $messageError . " Error loading file: " . $e->getMessage();
      $functions = new Functions;
      $functions->show_error($messageError);
      break;
    }
    ...
    if (!isset($objWorksheet)) {
      $messageError = $messageError . " Import can't be continued, because the sheet name is not found in your file. Please cross-check again.";
      break;
    }
    for ($row = $startRow; $row <= $startRow; $row++) {
      for ($col = $col_start; $col <= $col_end; $col++) {
        $header[$col] = trim($objWorksheet->getCellByColumnAndRow($col, $row)->getValue());
      }
    }
    $startRow++;
    for ($row = $startRow; $row <= $startRow + $loopChunkSize; $row++) {
      for ($col = $col_start; $col <= $col_end; $col++) {
        $result[$row][$header[$col]] = trim($objWorksheet->getCellByColumnAndRow($col, $row)->getValue());
      }
      $nourut++;
    }
...
function parseXLS_xml($fileName, $sheet_name, $row_start, $row_end, $col_start, $col_end) {
  $mixed = parseXLS($fileName, $sheet_name, $row_start, $row_end, $col_start, $col_end);
...
function convertXLStoPDF($fileNameSource, $fileNameTarget) {
  echo date('H:i:s'). " Convert <a href='$fileNameSource' target='_blank'>$fileNameSource</a> starting ...<br>";
  $inputFileType = PHPExcel_IOFactory::identify($fileNameSource);
  ...
  $objReader->setLoadSheetsOnly(array("Policy Management"));
  $objPHPExcel = $objReader->load($fileNameSource);
  $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'PDF');
  $objWriter->writeAllSheets();
  $objWriter->save($fileNameTarget);
  echo date('H:i:s') . " Done writing <a href='$fileNameTarget' target='_blank'>$fileNameTarget</a>, Peak memory usage: " . (memory_get_peak_usage(true) / 1024 / 1024) . " MB <br>";  
}
