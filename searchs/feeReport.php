...
//            alert('tes');
            ...
            $('#dataContent').load('reports/fee-report.php', {
				searchAccount: $('select[id="searchAccount"]').val(),
    ...
    <div class="offset3 span3">
        <form class="form-horizontal" id="searchForm" method="post">
            <div class="control-group">
                <label class="control-label" for="searchAccount">Account</label>
                <div class="controls">
                    <?php 
                    createCombo("SELECT * FROM account WHERE account_no IN (521000, 520900)", "", "", "searchAccount", "account_no", "account_name", 
                                "", "", "", 7);
                    ?>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="searchStockpile">Stockpile</label>
                <div class="controls">
                    <?php 
                    createCombo("SELECT s.stockpile_name, CONCAT(s.stockpile_code, ' - ', s.stockpile_name) AS stockpile_full
                                FROM user_stockpile us
                                INNER JOIN stockpile s
                                    ON s.stockpile_id = us.stockpile_id
                                WHERE us.user_id = {$_SESSION['userId']}
                                ORDER BY s.stockpile_code ASC, s.stockpile_name ASC", "", "", "stockpileId", "stockpile_name", "stockpile_full", 
                                "", "", "", 7);
                    ?>
                </div>
            </div>
