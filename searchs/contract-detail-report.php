...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$('#searchVendorId').change(function() {
            resetcontract_no('contract_no');
            //$('#slipPayment').hide();
            //$('#summaryPayment').hide();
            
            if(document.getElementById('searchVendorId').value != '') {
                resetcontract_no(' ');
                setcontract_no($('select[id="searchVendorId"]').val());
            }
        });
	});
		
	function resetcontract_no(text) {
        document.getElementById('contract_no').options.length = 0;
        var x = document.createElement('option');
        x.value = '';
        x.text = '-- Please Select' + text + '--';
        document.getElementById('contract_no').options.add(x);
    }
    
    function setcontract_no(vendorId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'getNumberContract',
                    vendorId: vendorId
            },
...
//            alert('tes');
 $.blockUI({ message: '<h4>Please wait...</h4>' }); 
            $('#dataContent').load('reports/contract-detail-report.php', {
				...
				contract_no: $('select[id="contract_no"]').val()				
...
<div class="row" style="background-color: #f5f5f5; 
    ...
    <div class="offset3 span3">
        <form class="form-horizontal" id="searchForm" method="post">
			...
			<div class="control-group">
                <label class="control-label" for="contract_no">Contract No </label>
                <div class="controls">
                    <?php
						createCombo("", "", "", "contract_no", "po_pks_id", "contract_no", "", 1, "select2combobox100", 1, "multiple");
					?>
                </div>
            </div>
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    createCombo("SELECT s.stockpile_id, CONCAT(s.stockpile_code, ' - ', s.stockpile_name) AS stockpile_full
                                ...
                                "", 7, "select2combobox100", 1, "multiple");
                    ?>
                </div>
            </div>
            <div class="control-group">
