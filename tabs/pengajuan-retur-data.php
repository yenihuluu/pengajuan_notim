    ...
    <div class="row-fluid" style="margin-bottom: 7px;">
        ...
        <?php if ($method == 'UPDATE') { ?>
            <div class="span3 lightblue">
                ...
                <?php
                createCombo("SELECT transaction_id, slip_no FROM transaction t LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = t.stockpile_contract_id WHERE sc.stockpile_id = {$stockpileId} AND DATE_FORMAT(t.transaction_date,'%d/%m/%Y') = '{$tanggalNotim}'", $slipLama, "", "slipLama", "transaction_id", "slip_no", "", "", "select2combobox100", 1);
                ?>
            </div>
        <?php } elseif ($method == 'APPROVE' || $method == 'FINISH') { ?>
            <div class="span3 lightblue">
                <label>Slip No <span style="color: red;">*</span></label>
                <?php
                createCombo("SELECT transaction_id, slip_no FROM transaction t LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = t.stockpile_contract_id WHERE sc.stockpile_id = {$stockpileId} AND DATE_FORMAT(t.transaction_date,'%d/%m/%Y') = '{$tanggalNotim}'", $slipLama, "$disabledProperty", "slipLama", "transaction_id", "slip_no", "", "", "select2combobox100", 1);
                ?>
            </div>
        <?php } else { ?>
        ...
        <?php if ($method == 'FINISH') { ?>
            ...
            <?php if ($stockpileId != '' && $tanggalNotimBaru != '') { ?>
                <div class="span3 lightblue">
                    ...
                    <?php
                    createCombo("SELECT transaction_id, slip_no FROM transaction t LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = t.stockpile_contract_id WHERE sc.stockpile_id = {$stockpileId} AND DATE_FORMAT(t.transaction_date,'%d/%m/%Y') = '{$tanggalNotimBaru}'", $slipBaru, "", "slipBaru", "transaction_id", "slip_no", "", "", "select2combobox100", 1);
                    ?>
                </div>
            <?php } else { ?>
                <div class="span3 lightblue">
...
AND t.transaction_id = {$nomorSlip} and t.fc_payment_status = 0";
$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
        ...
        if ($result !== false && $result->num_rows == 1) {
            ...
            // </editor-fold>
             ...
             <center><h3 style="color:red">No Slip sudah di ajukan</h3></center>
        <?php }
        ?>
