...
if(isset($_POST['invoiceId']) && $_POST['invoiceId'] != '') {
    ...
    $sql = "SELECT invoice_no, invoice_no2, invoice_tax, stockpileId, tax_date AS tax_date2, invoice_date AS invoice_date2,
CASE WHEN DATE_FORMAT(tax_date, '%d/%m/%Y') = '00/00/0000' THEN '' ELSE DATE_FORMAT(tax_date, '%d/%m/%Y') END AS tax_date,
CASE WHEN DATE_FORMAT(invoice_date, '%d/%m/%Y') = '00/00/0000' THEN '' ELSE DATE_FORMAT(invoice_date, '%d/%m/%Y') END AS invoice_date,
remarks 
FROM invoice
