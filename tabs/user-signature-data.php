...
if(isset($_POST['userId']) && $_POST['userId'] != '') {
	...
	$sql1 = "SELECT us.*
            ...
            WHERE us.user_id = {$userId} order by user_signature_id DESC limit 1
            ";
    $resultData1 = $myDatabase->query($sql1, MYSQLI_STORE_RESULT);
    
    if($resultData1 !== false && $resultData1->num_rows > 0) {
        $rowData1 = $resultData1->fetch_object();
        $signature = $rowData1->signature;
...
<form id="importForm" method="post" enctype="multipart/form-data" action="./data_processing.php">
    <input type="hidden" name="action" id="action" value="import_user_signature" />
    <input type="hidden" id="userId" name="userId" value="<?php echo $userId; ?>" />
    <!--<input type="hidden" name="MAX_FILE_SIZE" id="MAX_FILE_SIZE" value="5000000">-->
    
    <div class="row-fluid">   
        <div class="span4 lightblue">
            <img src="import/signature/<?php echo $signature; ?>" border="0"/>
        </div>
        </div>
    
    
    <!--<div class="row-fluid">-->
        <label class="control-label" for="imagefile">Image (Acceptable format is JPG/PNG file) <span style="color: red;">*</span></label>
