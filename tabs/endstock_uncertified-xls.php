...
$fileName = "End Stock Report Uncertified " . $periodFull . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "M";
...
ROUND((SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1
	...
	SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status = 2
...
ROUND(SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 
	...
	(SELECT CASE WHEN tx.tax_category = 1 THEN fc.`price_converted` / ((100 - f.pph)/100)
...
ROUND(SUM(COALESCE(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 
	THEN (SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) / ((100 - f.pph)/100)
	ELSE (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) END
	FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
	LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id WHERE fc.freight_cost_id = t.`freight_cost_id`)
	ELSE 0 END,0)) - 
	SUM(COALESCE((CASE WHEN t.transaction_type = 1 AND t.delivery_status = 2
	THEN (SELECT SUM(percent_taken / 100) * 
	(SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) / ((100 - f.pph)/100)
	ELSE (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) END
	FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
	LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id WHERE fc.freight_cost_id = t.`freight_cost_id`)
...
WHERE 1=1 {$whereAvailableProperty} AND t.ggl = 0 AND t.rsb = 0
...
ROUND(SUM(d.`quantity` * 
(SELECT c.price_converted FROM contract c 
	...
	WHERE t.`transaction_id` = d.`transaction_id`)),2) AS pks_amount,
ROUND(SUM((
	...
	LEFT JOIN `transaction` t2 ON t2.freight_cost_id = fc.`freight_cost_id` WHERE t2.transaction_id = d.transaction_id)) -
	...
	ELSE 1 END) * 
	(SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = d.transaction_id) / ((100 - f.pph)/100)
	ELSE (SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = d.transaction_id) END
	FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
	LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id 
...
WHERE 1=1 {$whereAvailableProperty2} AND t.`slip_retur` IS NULL AND t.`notim_status` = 0 
AND t.ggl = 0 AND t.rsb = 0
GROUP BY  SUBSTRING(t.slip_no,1,3)
