...
if(isset($_POST['purchaseId']) && $_POST['purchaseId'] != '') {
   ...
   // $readonlyProperty = 'readonly';
    ...
    $sql = "SELECT contract_id
            FROM purchasing
            WHERE purchasing_id = {$purchaseId}
            ";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#contractDataForm").validate({
            ...
            messages: {               
				contractId: "Contract ID is a required field."
            },
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (parseInt(returnVal[4]) != 0)	//if no errors
                        {
                            ...
                            if (returnVal[1] == 'OK') {
                                document.getElementById('purchaseId').value = returnVal[3];
                                
                                $('#dataContent').load('forms/purchasing.php', { purchaseId: returnVal[3] }, iAmACallbackFunction2);
...
<form method="post" id="contractDataForm">
    <input type="hidden" name="action" id="action" value="purchasing_data" />
    <input type="hidden" name="purchaseId" id="purchaseId" value="<?php echo $purchaseId; ?>" />
    <div class="row-fluid">   
        
        <div class="span4 lightblue">
            <label>Contract Id</label>
            <input type="text" class="span12" tabindex="13" id="contractId" name="contractId" value="<?php echo $contractId; ?>">
        </div>
    </div>
