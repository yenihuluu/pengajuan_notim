...
if (isset($_POST['kodeMutasi']) && $_POST['kodeMutasi'] != '') {
    ...
    //TABLE PALING BAWAH
    $sql1 = $myDatabase->query("select mh.*, sum(t.send_weight) as total_sendWeight, sum(t.netto_weight) as total_netto, t.* from mutasi_header mh 
    left join `transaction` t on t.mutasi_id = mh.mutasi_header_id where mh.mutasi_header_id = {$kodeMutasi}", MYSQLI_STORE_RESULT);
    $rowData = $sql1->fetch_object();
    $stockpileId = $rowData->stockpile_to;
	$totalSendWeight = $rowData->total_sendWeight;
    ...
    $nettoStockpile = $rowData->netto_weight;
    ...
    $sql = "select st.*, st.send_weight as send_weightST,st.netto_weight as netto_weightST, CONCAT (s.stockpile_code, ' - ', s.stockpile_name) as stockpile_from, CONCAT (ss.stockpile_code, ' - ', ss.stockpile_name) as stockpile_to, c.contract_no as contract_no, c.price,
            mh.kode_mutasi as destination_code, t.* , t.netto_weight as netto_weightTrx
...
//                    $unitCost = $unitPrice + $mutasiValueTon + $shrinkValueTon;
                    ...
                    $mutasiCost = $mutasiValueTon * $nettoTransit;
