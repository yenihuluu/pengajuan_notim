    ...
    // } else
     if ($_POST['_method'] == 'CANCEL') {
        ...
        if($resultA !== false) {
          ...
          //  insertGeneralLedger($myDatabase, 'RETURN INVOICE NOTIM', $invId);
        ...
        $pgId = $_POST['pgId'];
        $sqlUpdatePG = "UPDATE `pengajuan_general` SET status_pengajuan = 4 , invoice_id = NULL, reject_remarks = '{$rejectRemarks}', reject_date = STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s') WHERE pengajuan_general_id = {$pgId}";
        echo $sqlUpdatePG;
        $updatePG = $myDatabase->query($sqlUpdatePG, MYSQLI_STORE_RESULT);
...
//                                $requestDate = $rowData->date;
                            ...
                            while ($row = $resultPODetail->fetch_object()) {
                                ...
                                $sql = "INSERT INTO `pengajuan_general_detail` (po_detail_id,pg_id,invoice_method_detail, type, account_id, poId, 
                                        vendor_type ,vendor_name,general_vendor_id, shipment_id, stockpile_remark, qty, price, termin, amount, 
                                        amount_converted, currency_id, exchange_rate, ppnID, ppn, ppn_converted, pphID, pph, pph_converted,
                                         tamount, tamount_converted, dp_amount, notes,gv_bank_id, entry_by, entry_date) VALUES ("
                            ...
                            if ($resultA !== false && $resultA->num_rows > 0) {
                                ...
                                while ($rowA = $resultA->fetch_object()) {
                                    $totalDpp = $totalDpp + ($rowA->amount_converted * $tempTermin);
                                    $totalQty = $totalQty + $rowA->qty;
                                    $totalPrice = $totalPrice + $rowA->price;
                                    $totalPPn = $totalPPn + ($rowA->ppn_converted * $tempTermin);
                                    $totalPPh = $totalPPh + ($rowA->pph_converted * $tempTermin);
                ...
                //----------------------------------------------------------------UPDATE------------------------------------------------------------------------------------------------------
                    ...
                    if ($result !== false) {
                        ...
                        $updateLog = "UPDATE `logbook_new` SET status1 = 0 WHERE pgeneral_id = {$pgId}";
                ...
                //---------------------------------------------------------APPROVE INVOICE-------------------------------------------------------------------------------------------------
                   ...
                   $sqlA = "SELECT type, account_id FROM pengajuan_general_detail WHERE pg_id = {$pgId}";
                   $resultA = $myDatabase->query($sqlA, MYSQLI_STORE_RESULT);
                   $null = 0;
                   $notNull = 0;
                   if($resultA !== false ){
                        ...
                        while ($rowA = $resultA->fetch_object()) {
                            if($rowA->type != '' && $rowA->account_id != ''){
                                $notNull = $notNull + 1;
                            }elseif($rowA->type == '' && $rowA->account_id == ''){
                                $null = $null + 1;
                            }
                        }
                ...
                if($notNull > 0 && $null == 0){ //jika detail sudah di isi
                   ...
                   while ($rowPG = $pg->fetch_object()) {
                       ...
                       $sqlInsertPg = "INSERT INTO `invoice` (invoice_date,request_date, tax_date, invoice_method,po_id, "
                                       ...
                                       ." {$rowPG->total_qty}, {$rowPG->total_price}, {$rowPG->total_dpp}, {$rowPG->total_amount}, {$rowPG->total_pph}, {$rowPG->total_ppn})";
                       $resPG = $myDatabase->query($sqlInsertPg, MYSQLI_STORE_RESULT);
                       $invId = $myDatabase->insert_id;

                       $sql1 = "UPDATE pengajuan_general SET invoice_method = {$invoiceMethod} WHERE pengajuan_general_id = {$pgId} ";
                   ...
                   while ($rowPGD = $pgd->fetch_object()) {
                       ...
                       $codePrediksi = isset($rowPGD->prediksi_code_detail) ? $rowPGD->prediksi_code_detail : 0;
                       $prediksiAmount = isset($rowPGD->prediksi_amount) ? $rowPGD->prediksi_amount : 0;
                       $prediksiId= isset($rowPGD->prediction_detail_id) ? $rowPGD->prediction_detail_id : 0;



                       $sqlID = "INSERT INTO `invoice_detail` (pgd_id,invoice_id,invoice_method_detail, type, account_id, poId, general_vendor_id, shipment_id,
                                ...
                                . "{$rowPGD->pgd_id},{$invId},{$invoiceMethod}, {$rowPGD->type}, {$rowPGD->account_id},{$pgdPOID}, {$rowPGD->general_vendor_id}, "
                                ...
                                ." {$rowPGD->exchange_rate} ,'{$rowPGD->ppnID}', '{$rowPGD->ppn}',{$rowPGD->ppn_converted},'{$rowPGD->pphID}', '{$rowPGD->pph}', '{$rowPGD->pph_converted}', "
...
// </editor-fold>
    ...
    //$invoiceId = 7322;
    // </editor-fold>
    $sqlNotim = "SELECT * FROM `accrue_prediction_detail` WHERE prediction_detail_id IN (335, 336)";
    ...
    //$invoiceId = 7322;
    // </editor-fold>
    $sqlNotim = "SELECT * FROM `invoice_detail` WHERE invoice_detail_id IN (54103, 54104)";
    //$sqlNotim = "SELECT * FROM invoice_detail WHERE entry_date BETWEEN '2020-05-01' AND '2020-05-31'";
    $resultNotim = $myDatabase->query($sqlNotim, MYSQLI_STORE_RESULT);
    if ($resultNotim !== false && $resultNotim->num_rows > 0) {
        while ($rowNotim = $resultNotim->fetch_object()) {
    ...
    if (isset($_POST['_method']) && $_POST['_method'] == 'DELETE') {
        ...
        if (isset($_POST['pgdId']) && $_POST['pgdId'] != '') {
            ...
            if($invoiceType == 4){
                ...
                $wherePrediksi = " prediction_detail_id = {$_POST['prediksiDetailId']}, 
                                   prediksi_code_detail = '{$_POST['codePrediksi']}',
                                  biaya_id = {$_POST['tipeBiayaId']}, 
            ...
            if ($result !== false) {
                ...
                if (isset($_POST['checkedSlips2'])) {
                    ...
                    for ($i = 0; $i < sizeof($checks2); $i++) {
                        if ($checks[$i] != '') {
                            if ($slipNos2 == '') {
                                $slipNos2 .= '(' . $_POST['pgdId'] . ',' . $checks[$i] . ',' . $checks2[$i] . ')';
                            } else {
                                $slipNos2 .= ',' . '(' .$_POST['pgdId'] . ',' . $checks[$i] . ',' . $checks2[$i] . ')';
                            }
                        }
                ...
                // echo $slipNos2;
                $sqlD = "INSERT INTO invoice_dp (pengajuan_detail_id, invoice_detail_dp, amount_payment) VALUES {$slipNos2}";
                $resultD = $myDatabase->query($sqlD, MYSQLI_STORE_RESULT);
                if ($resultD === false) {
                    echo 'salah';
                    //Update DP amount 
                    ...
                    if ($resultDP !== false && $resultDP->num_rows > 0) {
                        ...
                        if ($rowDP->ppn == 0) {
                            ...
                            $sql2 = "UPDATE `pengajuan_general_detail` SET dp_amount = {$downPayment} WHERE pgd_id = {$_POST['pgdId']}";
                            $result2 = $myDatabase->query($sql2, MYSQLI_STORE_RESULT);
                    }
                }
                ...
                if (count(array_unique($gvId)) === 1) {
                    ...
                    if ($result1 !== false && $result1->num_rows > 0) {
                        while ($row1 = $result1->fetch_object()) {
                            ...
                            $sql = "INSERT INTO `pengajuan_general_detail` (pg_id,invoice_method_detail, type,account_id, poId, vendor_type,vendor_name,
...
//                                $resultN = $myDatabase->query($sqlN, MYSQLI_STORE_RESULT);
            ...
            if ($qty != '' && $price != '' && $termin != '' && $amount != '' && $currencyId != '' && $boolShipment1 && $boolPO) {
                ...
                $t_amount = ($amount2 * ($termin/100)) + $ppn2 - $pph2;
                $tamount = $t_amount - $dp_total;
                $amountConverted = $exchangeRate * ($amount2 * ($termin/100));
                $ppnConverted = $exchangeRate * $ppn2;
                $pphConverted = $exchangeRate * $pph2;
                $tamountConverted = $exchangeRate * $tamount;
                $amount3 = $amount2 * ($termin/100);
                ...
                $sql = "INSERT INTO `pengajuan_general_detail` (pg_id,gv_bank_id,invoice_method_detail, poId, vendor_type,vendor_name,general_vendor_id, "
                                   ...
                                   ." tamount_converted, dp_amount, notes, entry_by, entry_date, uom_id, checked_ppn) VALUES ("
                                    . "{$pgId},{$gvBankId},{$invoiceMethodDetail},{$poId},'{$vendorType}','{$vendorName}', "
                                    ." {$generalVendorId}, {$shipmentId1}, {$stockpileId2}, $qty,$price,$termin, {$amount3}, {$amountConverted}, "
                                    ." {$currencyId}, '{$exchangeRate}', {$ppnID} ,'{$ppn2}', '{$ppnConverted}', {$pphID}, '{$pph2}', '{$pphConverted}', "
                                    ." '{$tamount}', '{$tamountConverted}', '{$dpAmount}', '{$notes}',{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), {$uomId}, {$checkValue})";
    ...
    //      $returnValue .= '<a href="#" id="delete|pg|' . $row->pgd_id . '" role="button" title="Delete" onclick="deleteInvoiceDetail(' . $row->pgd_id . ');"><img src="assets/ico/gnome-trash.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>';
        ...
        $returnValue .= '<tfoot>';
        $returnValue .= '<tr>';
        $returnValue .= '<td colspan="15" style="text-align: right;">Grand Total</td>';
...
INNER JOIN contract c ON sc.`contract_id` = c.`contract_id` WHERE sc.`quantity` > 0 AND c.`contract_no` = '$row->contract_no'";
        ...
        if ($row->general_ledger_id != '' && ($row->general_ledger_module == 'NOTA TIMBANG' || ($row->general_ledger_module == 'CONTRACT' && ($row->general_ledger_for == 1 || $row->general_ledger_for == 2)) || $row->general_ledger_module == 'STOCK TRANSIT' || $row->general_ledger_module == 'CONTRACT ADJUSTMENT' || $row->general_ledger_module == 'JURNAL MEMORIAL'  || $row->general_ledger_module == 'JURNAL ACCRUE')) {
            $debit_amount = $row->amount;
    ...
    if ($accrueId != 'NULL') {
        ...
        if ($result !== false && $result->num_rows == 1) {
			...
			$journal_status = $row->journal_status;
				...
				$sqlAccount1 = "SELECT account_id, account_no, account_name FROM account WHERE account_type = 4 AND account_no IN (220100, {$accountNo})";
                        ...
                        if($boolReverse){
                            ...
                            if ($totalAmount != 0 && $totalAmount != '') {
                                if ($rowAccount1->account_no == $accountNo) {
                                    if ($totalAmount < 0) {
                                        $insertValues .= "(1, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, 'Reverse Journal', {$totalAmount})";
                                    } else {
                                        $insertValues .= "(2, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, 'Reverse Journal', {$totalAmount})";
                                    }
                            ...
                            if ($totalAmount != 0 && $totalAmount != '') {
                                if($journal_status == 2){
                                    if ($rowAccount1->account_no == $accountNo) {
                                        if ($totalAmount < 0) {
                                            $insertValues .= "(1, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, 'Cancel Journal', {$totalAmount})";
                                        } else {
                                            $insertValues .= "(2, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, 'Cancel Journal', {$totalAmount})";
                                        }
                                    } elseif ($rowAccount1->account_no == 220100) {
                                        if ($totalAmount < 0) {
                                            $insertValues .= "(2, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, 'Cancel Journal', {$totalAmount})";
                                    ...
                                    if ($rowAccount1->account_no == $accountNo) {
                                        if ($totalAmount < 0) {
                                            $insertValues .= "(2, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, NULL, {$totalAmount})";
                                        } else {
                                            $insertValues .= "(1, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, NULL, {$totalAmount})";
                                        }
                                    } elseif ($rowAccount1->account_no == 220100) {
                                        if ($totalAmount < 0) {
                                            $insertValues .= "(1, '{$source}', NULL, NULL, 15, {$contractId}, {$invoiceId}, {$transactionId}, 0, {$paymentId}, {$jurnalId}, {$accrueId},0, 0,{$rowAccount1->account_id}, NULL, {$totalAmount})";
