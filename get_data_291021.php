...
switch ($_POST['action']) {
   ...
   case "setSlipCurah":
    ...
    break;
    case "getGVprediksi":
        getGVprediksi($_POST['shipmentId'], $_POST['accountId'] );
    break;
    case "getAccountInvoicePrediksi":
        getAccountInvoicePrediksi($_POST['shipmentId1']);
    break;
    case "getPoInvoice":
    ...
    case "setPrediksiDetail":
        setPrediksiDetail($_POST['prediksiId']);
    break;
    ...
    case "getShipmentInvoicePrediksi":
        getShipmentInvoicePrediksi($_POST['invoiceType']);
    break;
    case "getPrediksiId":
        getPrediksiId($_POST['shipmentId'], $_POST['tipeBiayaId'], $_POST['generalVendorId']);
    break;
    ...
    break;
    case "addNewData":
        addNewData($_POST['headerCostingID'], $_POST['stockpileId'], $_POST['qtyVessel'], $_POST['prediksiId']);
    break;
    case "getShipmentGGL_SRB1":
        getShipmentGGL_SRB1($_POST['shipmentId']);
    break;
    case "insertTempCosting":
        insertTempCosting($_POST['headerCostingID']);
    break;
    ...
    break;
    case "hitungCosting1":
        hitungCosting1($_POST['qtyVessel']);
    break;
...
function insertTempCosting($headerCostingID)
{
    global $myDatabase;
    $whereProperty = '';
    $whereProperty2 = '';
    ...
    if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_object()) {
               ...
               $sqlA = "INSERT INTO `temp_mst_costing` (id_mst, generate_code, nama_biaya, general_vendor_id, account_id, max_type, maxused, max_charge, max_chargeP,  "
                        ...
                        . "qty_value, price, total_amount, kurs, rupiah, stockpile_id, uom_id) VALUES ("
                        ...
                        . "{$QtyType}, {$row->priceMT}, {$totalAmount}, {$row->exchange_rate}, {$inRupiah}, {$row->stockpile_id}, {$row->uom})";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            $sqlA = "INSERT INTO `temp_mst_costing` (header_prediksi, id_mst, prediksi_detail_id, generate_code, nama_biaya, general_vendor_id, account_id, max_type, maxused, max_charge, max_chargeP,  "
                    . "min_type, minused, min_charge, min_chargeP, qty_type_id, price_type_id, "
                    . "qty_value, price, total_amount, kurs, rupiah, stockpile_id, uom_id, status, journal_status) VALUES ("
                    . "{$row->prediction_id}, {$row->mcd_id}, {$row->prediction_detail_id}, '{$row->generate_code_detail}', {$row->cost_name}, {$row->general_vendor_id}, '{$row->account_id}', {$row->max_type}, {$row->maxused}, {$row->max_charge}, "
                    . "{$row->max_chargeP}, {$row->min_type}, {$row->minused}, {$row->min_charge}, {$row->min_chargeP},  {$row->qty_type_id}, {$row->price_type_id}, "
                    . "{$row->qty}, {$row->priceMT}, {$row->total_amount}, {$row->exchange_rate}, {$row->in_rupiah}, {$row->stockpile_id}, {$row->uom}, {$row->status}, {$row->journal_status})";
            $resultA = $myDatabase->query($sqlA, MYSQLI_STORE_RESULT);
      //  echo $sqlA;
        }
    }
...
function getTempCosting(){
    ...
    $sql = "SELECT tmc.*, tmc.id AS tempID, tmc.id_mst AS costingId, 
            ...
            CASE WHEN tmc.qty_type_id = 1 THEN 'Vessel'
                WHEN tmc.qty_type_id = 2 THEN 'Tongkang' ELSE 'Timbangan' END AS coaType, 
        ...
        if ($result->num_rows > 0) {
            ...
            $returnValue .= '<tbody>';
                ...
                while ($row = $result->fetch_object()) {
                    ...
                    if($row->journal_status == 2){
                        $returnValue .= '<td><img src="assets/ico/cancel.png" width="18px" height="18px" style="margin-bottom: 5px;" /></td>'; 
                    }else if($row->journal_status == 1){
                        $returnValue .= '<td color: green;"> <img src="assets/ico/checklist.png" width="18px" height="18px" style="margin-bottom: 5px;" /></td>'; 
                    }else{
                        $returnValue .= '<td></td>';
                    }
                    ...
                    $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->price, 0, ".", ",") . '</td>';
                    $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->total_amount, 0, ".", ",") . '</td>';
                    $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->kurs, 2, ".", ",") . '</td>';
                    $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->rupiah, 2, ".", ",") . '</td>';
...
//HITUNG QTY VESSEL
function hitungCosting1($qtyVessel){
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            if($row->qty_type_id == 1){ //Vessel
                ...
                $inRupiah = $totalAmount * $row->kurs;
                $sqlA = "UPDATE temp_mst_costing set maxused = {$maxused}, minused = {$minused}, qty_value = {$tempQtyV}, total_amount = {$total}, rupiah = {$inRupiah} WHERE id = {$tempIDV}";
                $resultA = $myDatabase->query($sqlA, MYSQLI_STORE_RESULT);           
            }
        }
    }else{
...
function addNewData($headerCostingID, $stockpileId, $qtyVessel, $prediksiId)
{
    global $myDatabase;
    $whereProperty = '';
    $whereProperty2 = '';
    ...
    $sql = "SELECT mc.*, mc.mcd_id AS costingId, sp.stockpile_name, sp.stockpile_code as spCode, gv.general_vendor_name,  
            ...
            CASE WHEN mc.qty_type = 1 THEN 'Vessel' ELSE 'Tongkang' END AS coaType, 
            u.uom_type, cur.currency_code, tb.tipe_biaya, 
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
                ...
                $returnValue .= '<td style="width: 10%;">' . $row->general_vendor_name . '</td>';
                $returnValue .= '<td style="width: 10%;">' . $row->accountName . '</td>';     
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($maxused, 0, ".", ",") . ' </td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($minused, 0, ".", ",") . '</td>';
                $returnValue .= '<td style="width: 10%;">' . $row->coaType . ' </td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($QtyType, 0, ".", ",") . '</td>'; 
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->priceMT, 0, ".", ",") . ' </td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($totalAmount, 0, ".", ",") . '</td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->exchange_rate, 2, ".", ",") . '</td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($inRupiah, 2, ".", ",") . '</td>';
...
function getShipmentCosting($stockpileId)
{
    ...
    $sql = "SELECT DISTINCT(sh.shipment_id) as shipmentId, sh.shipment_no
            ...
            WHERE sp.stockpile_id = {$stockpileId} 
...
function getSalesPrediksi($shipmentId)
{
    ...
    if ($result->num_rows == 1) {
        ...
        $returnValue = '|' .  number_format($qty, 2, ".", ",") . '|' . $row->customer_id . '|' .$row->customer_name . '|' . $row->mother_vessel . '|' . $row->pebDate . '|' . $row->currency_id . '|' . $row->currency_code . '|' .  number_format($row->exchangerRate, 2, ".", ",") . '|' .$row->sales_id;
...
function getLoadingPort($headerCostingID)
{
    ...
    if ($result->num_rows == 1) {
        ...
        $returnValue = '|' . $row->stockpileId . '|' .$row->stockpile_name;
...
LEFT JOIN account c ON a.account_id = c.account_id WHERE a.mutasi_header_id = {$mutasiId} AND a.status = 0";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            $returnValue .= '<td style="text-align: right; width: 0%;">' . number_format($row->qty, 2, ".", ",") . '</td>';
...
function getPrediksiId($shipmentId, $tipeBiayaId, $generalVendorId)
{
    global $myDatabase;
    $returnValue = '';
...
function getShipmentInvoicePrediksi($invoiceType)
{
    global $myDatabase;
    $returnValue = '';
    //$whereProperty = '';
    ...
    $sql = "SELECT a.shipment_id, CONCAT(a.shipment_no, ' - ', SUBSTR(a.`shipment_code`,-1)) AS shipment_no, b.`stockpile_id`  
            ...
            WHERE b.`invoice_type` = {$invoiceType} AND b.status <> 1 ORDER BY shipment_id DESC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
CASE WHEN idUOM IS NOT NULL THEN (SELECT uom_type FROM uom WHERE idUOM = id.idUOM) ELSE '-' END AS uom, id.prediksi_amount
...
WHERE id.invoice_id IS NULL AND id.entry_by = {$_SESSION['userId']} ORDER BY id.invoice_detail_id ASC";
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<thead><tr>
                                    ...
                                    <th>Amount</th>
                                    <th>PPN</th><th>PPh</th><th>Down Payment</th><th>Total Amount</th><th>Action</th></tr></thead>';
        $returnValue .= '<tbody>';
...
function setInvoiceDP($generalVendorId, $ppn1, $pph1)
{
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            $dppTotalPrice = $row->amount;
            $prediksiAmount = $row->prediksi_amount;
            ...
            $total2 = ($row->total_dp + $dp_ppn) - $dp_pph;
            $total_dp = $total - $total2;
            $totalPrediksi_dp = $prediksiAmount - ($row->total_dp);
...
function setPrediksiDetail($prediksiId)
{
    ...
    $sql = "SELECT CONCAT(a.account_no, ' - ', a.account_name) AS accountName, apd.mcd_id,
                CASE WHEN c.qty_type = 1 THEN 'Vessel' ELSE 'Tongkang' END AS jenisQty,
                apd.max_charge, apd.min_charge, apd.qty, apd.priceMT, apd.total_amount, ap.prediction_code,
                apd.exchange_rate, apd.in_rupiah, s.stockpile_name as spremarks, apd.generate_code_detail, apd.prediction_detail_id FROM accrue_prediction ap
            LEFT JOIN accrue_prediction_detail apd ON apd.`prediction_id` = ap.`prediction_id`
            LEFT JOIN mst_costing_detail c ON c.`mcd_id` = apd.`mcd_id`
            LEFT JOIN account a ON a.`account_id` = c.`account_id`
            LEFT JOIN stockpile s ON s.stockpile_id = apd.stockpile_id
            WHERE apd.prediction_detail_id = {$prediksiId} AND apd.status in (0,2)";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            $returnValue .= '<td style="width: 15%;">' . $row->accountName . '</td>';
            $returnValue .= '<td style="text-align: right; width: 8%;">' . number_format($row->max_charge, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 8%;">' . number_format($row->min_charge, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="width: 5%;">' . $row->jenisQty . '</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->qty, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->priceMT, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->total_amount, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 8%;">' . number_format($row->exchange_rate, 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($row->in_rupiah, 2, ".", ",") .'</td>';
...
function getAccountInvoice($invoiceType)
{
    ...
    $sql = "SELECT acc.account_id, CONCAT(acc.account_name) AS account_full
            FROM account acc {$joinProperty}
            WHERE acc.status = 0 AND acc.account_type = {$invoiceType} AND acc.account_no NOT IN (210105,150100,130000,150000,110100,605000,520500,700000)";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function getAccountInvoicePrediksi($shipmentId1)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
    ...
    $sql = "SELECT acc.account_id, CONCAT(acc.account_name) AS account_full
            ...
            WHERE ap.shipment_id = {$shipmentId1} AND apd.status IN (0,2)";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function getGVprediksi($shipmentId, $accountId)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
    ...
    $sql = "  SELECT gv.general_vendor_id, gv.general_vendor_name, tb.id as costId, tb.tipe_biaya
            FROM accrue_prediction ap
            LEFT JOIN accrue_prediction_detail apd ON apd.prediction_id = ap.prediction_id
            ...
            WHERE ap.shipment_id = {$shipmentId} AND apd.account_id = {$accountId} ";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            ...
            if ($returnValue == '') {
                $returnValue = '~' . $row->general_vendor_id . '||' . $row->general_vendor_name . '||' . $row->costId . '||' . $row->tipe_biaya;
            } else {
                $returnValue = $returnValue . '{}' . $row->general_vendor_id . '||' . $row->general_vendor_name .  '||' . $row->costId . '||' . $row->tipe_biaya;
            }
        }
    }
