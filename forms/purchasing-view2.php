...
$sql = "SELECT CONCAT(v.`vendor_code`, ' - ', v.vendor_name) AS vendor_name,
		...
		DATE_FORMAT(p.admin_input, '%d %b %Y %h:%m:%s') AS admin_input,pp.contract_no
		FROM purchasing p
		LEFT JOIN stockpile s ON s.`stockpile_id`=p.`stockpile_id`
		LEFT JOIN vendor v ON v.`vendor_id`=p.`vendor_id`
		left join po_pks pp on pp.purchasing_id=p.purchasing_id WHERE p.purchasing_id = {$purchasingId}";
