<?php if ($_POST['showImage'] === 'true') { ?>
    ...
    $(document).ready(function () {
        if (sessionStorage.getItem("file1") !== null) {
            var result = '<img id="base64image" src="' + sessionStorage.getItem("file1") + '"/>';
        } else {
    ...
    <?php } else { ?>
        ...
        function take_snapshot() {
            Webcam.snap(function (data_uri) {
                sessionStorage.setItem("file1", data_uri);
                $('#photoTicket').val(data_uri);
            });
            resetCam();
        }
        function ShowCam() {
