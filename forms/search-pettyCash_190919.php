...
LEFT JOIN invoice_detail id ON id.`general_vendor_id` = gv.`general_vendor_id`
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
WHEN p.payment_cash_id IS NOT NULL THEN (SELECT gv.bank_name FROM general_vendor gv LEFT JOIN payment_cash pc ON pc.general_vendor_id = gv.general_vendor_id WHERE pc.payment_id = p.payment_id LIMIT 1) 
...
LEFT JOIN invoice_detail id ON id.`general_vendor_id` = gv.`general_vendor_id`
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
WHEN p.payment_cash_id IS NOT NULL THEN (SELECT gv.branch FROM general_vendor gv LEFT JOIN payment_cash pc ON pc.general_vendor_id = gv.general_vendor_id WHERE pc.payment_id = p.payment_id LIMIT 1) 
...
LEFT JOIN invoice_detail id ON id.`general_vendor_id` = gv.`general_vendor_id`
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
WHEN p.payment_cash_id IS NOT NULL THEN (SELECT gv.account_no FROM general_vendor gv LEFT JOIN payment_cash pc ON pc.general_vendor_id = gv.general_vendor_id WHERE pc.payment_id = p.payment_id LIMIT 1) 
...
LEFT JOIN invoice_detail id ON id.`general_vendor_id` = gv.`general_vendor_id`
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
WHEN p.payment_cash_id IS NOT NULL THEN (SELECT gv.beneficiary FROM general_vendor gv LEFT JOIN payment_cash pc ON pc.general_vendor_id = gv.general_vendor_id WHERE pc.payment_id = p.payment_id LIMIT 1) 
...
LEFT JOIN invoice_detail id ON id.`general_vendor_id` = gv.`general_vendor_id`
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
WHEN p.payment_cash_id IS NOT NULL THEN (SELECT gv.swift_code FROM general_vendor gv LEFT JOIN payment_cash pc ON pc.general_vendor_id = gv.general_vendor_id WHERE pc.payment_id = p.payment_id LIMIT 1) 
...
pc.qty, pc.price, pc.termin, pc.amount, pc.ppn, pc.pph, pc.tamount, gv.general_vendor_name, c.po_no
...
WHERE pcdp.payment_cash_id IN ({$pc_id})";
                ...
                if($resultDP !== false) {
                 ...
                 $sqlDP = "SELECT p.amount_converted, p.original_amount, p.pph_amount FROM payment p 
                        WHERE p.freight_id = {$row->freight_id} AND p.payment_id = {$row->payment_id}";
                $resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
                if($resultDP !== false) {
                    $rowDP = $resultDP->fetch_object();
                    if($rowDP->original_amount < 0 ){
					$downPayment = ($rowDP->amount_converted - $rowDP->pph_amount) - ($rowDP->original_amount);
					}else{
...
ELSE COALESCE(SUM(t.freight_quantity), 0) END AS freight_quantity, fc.freight_id 
 FROM `transaction` t
						  INNER JOIN freight_cost fc ON t.`freight_cost_id` = fc.`freight_cost_id`
                ...
                $sql = "SELECT t.slip_no, t.freight_quantity AS quantity, fc.price, 
							...
							WHEN f.freight_id = 288 THEN t.send_weight * t.freight_price ELSE t.freight_quantity * t.freight_price  END
		...
		$downPayment = $downPayment + $rowDetail->dp_amount / 1.1;
            ...
            <tr>
                ...
                <td><div style="text-align: right;">(<?php echo number_format($pph_amount, 2, ".", ","); ?>)</div></td>
            </tr>
             <?php
...
?>
<button class="btn btn-warning" id="returnPC">Return</button>
<?php
}
?>
