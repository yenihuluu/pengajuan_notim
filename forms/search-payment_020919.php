...
$sql = "SELECT p.*, DATE_FORMAT(p.payment_date, '%d %b %Y') AS payment_date2, DATE_FORMAT(p.payment_date, '%Y-%m') AS edit_date, pcur.currency_code AS pcur_currency_code, 
...
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
            ...
            WHEN p.freight_id IS NOT NULL THEN f.account_no
...
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
            ...
            WHEN p.freight_id IS NOT NULL THEN f.swift_code
        ...
        <tr>
        	<?php if($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != ''){
				...
				if($row->payment_method == 2){
							$total = ($row->amount_journal - $row->pph_journal) + $row->ppn_journal;
						}else{
							$total = $row->original_amount;
						}
			    //}else if($row->ppn_amount_converted > 0 && $row->pph_amount_converted_converted > 0 ) {
                //$total = ($row->original_amount + $row->ppn_amount_converted)- $row->pph_amount_converted_converted;
			    //}else if($row->ppn_amount_converted > 0) {
                //$total = $row->original_amount + $row->ppn_amount_converted;
				//}else if($row->pph_amount_converted_converted > 0) {
                //$total = $row->original_amount - $row->pph_amount_converted_converted;
		        } elseif($row->general_vendor_id != '' && $row->pph_tax_id == 21){
				   $total =	($row->original_amount + $row->ppn_amount)+ $row->pph_amount_converted; 
				}
            ...
            <td width="2%">:</td>
            <td width="28%"><?php echo 'IDR' . ' ' . number_format($row->exchange_rate, 3, ".", ","); ?></td>
        ...
        <tbody>
            ...
            if($row->stockpile_contract_id != '') {
                $sql = "SELECT '' AS slip_no,  con.price, con.po_no, con.contract_no,
							CASE WHEN p.payment_type = 1 THEN con.adjustment
                            ...
                            WHEN p.payment_method = 1 AND p.payment_type = 1 THEN con.adjustment * con.price
...
id.qty, id.price, id.termin, id.amount, id.ppn, id.pph, id.tamount, gv.general_vendor_name, 
...
LEFT JOIN contract c ON c.contract_id = id.poId
			...
			if($row->payment_date >= '2018-08-20' ){
                $sqlDP = "SELECT SUM(idp.amount_payment) AS down_payment, SUM(id.ppn) AS ppn, SUM(id.pph) AS pph  FROM invoice_dp idp LEFT JOIN invoice_detail id ON id.`invoice_detail_id` = idp.`invoice_detail_dp`
WHERE idp.invoice_detail_id IN ({$iddps}) ";
                $resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
                if($resultDP !== false) {
                    $rowDP = $resultDP->fetch_object();
					if($rowDP->ppn == 0){
...
WHERE id.invoice_detail_dp IN ({$iddps}) AND id.invoice_method_detail = 2 AND id.invoice_detail_status = 1";
			...
			//echo $downPayment1;
                ...
                $sql = "SELECT t.slip_no, t.quantity AS quantity, t.unit_price, 
                            CASE WHEN p.payment_method = 1 THEN 
                                CASE WHEN p.payment_status = 0 THEN (t.quantity * t.unit_price) ELSE p.original_amount END
				...
				$sql = "select remarks from payment p where p.payment_id = {$row->payment_id}
                 ...
                 $sqlDP = "SELECT p.amount_converted, p.original_amount, p.pph_amount_converted, p.amount_journal FROM payment p 
                        WHERE p.freight_id = {$row->freight_id} AND p.payment_id = {$row->payment_id}";
                $resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
                if($resultDP !== false) {
                    $rowDP = $resultDP->fetch_object();
                    if($rowDP->original_amount < 0 ){
					$downPayment = ($rowDP->amount_converted - $rowDP->pph_amount_converted) - ($rowDP->amount_journal);
					}else{
					$downPayment = $rowDP->amount_converted - $rowDP->amount_journal;
					}
                }
				$sqlFQ = "SELECT 
...
ELSE COALESCE(SUM(t.freight_quantity), 0) END AS freight_quantity, fc.freight_id 
 FROM `transaction` t
                ...
                $sql = "SELECT t.slip_no, t.freight_quantity AS quantity, fc.price, 
                            CASE WHEN p.payment_method = 1 THEN 
                 ...
                 $sqlDP = "SELECT p.amount_converted, p.original_amount, p.pph_amount_converted FROM payment p 
                        WHERE p.vendor_handling_id = {$row->vendor_handling_id} AND p.payment_id = {$row->payment_id}";
                $resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
                if($resultDP !== false) {
                    $rowDP = $resultDP->fetch_object();
                    if($rowDP->original_amount < 0 ){
					$downPayment = ($rowDP->amount_converted - $rowDP->pph_amount_converted) - ($rowDP->original_amount);
					}else{
					$downPayment = $rowDP->amount_converted - $rowDP->original_amount;
					}
                }
                
                $sql = "SELECT t.slip_no, t.handling_quantity AS quantity, vhc.price, 
                ...
                $sql = "SELECT sh.shipment_code AS slip_no, 
                            ...
                            WHEN p.payment_method = 2 THEN pd.amount - p.ppn_journal
                            ELSE '' END AS original_amount,
                            CASE WHEN p.payment_method = 1 THEN 'Payment'
                            WHEN p.payment_method = 2 THEN 'Down Payment'
                            ELSE '' END AS payment_method2,
            ...
            <tr>
                ...
                <td>
                    ...
                    if($rowDetail->quantity != '') {
                        echo number_format($rowDetail->quantity, 3, ".", ",") . ' Kg'; 
                    } else if($row->qty != '' && $row->general_vendor_id != '') {
                        echo number_format($row->qty, 3, ".", ","); 
                    } else if($row->invoice_id != '') {
                        echo number_format($rowDetail->qty, 3, ".", ","); 
                ...
                <td>
                    ...
                    if($rowDetail->price != '') {
                        ...
                        echo number_format($rowDetail->price, 2, ".", ","); 
                    } else if($row->freight_id != '' && $row->payment_method = 2) {
                        echo number_format($row->price, 2, ".", ","); 
                    } else {
                        echo '-';
                ...
                %</td>
                <td><?php 
				    if($row->freight_id == 26){
					echo $row->payment_id; 
					}else{ 
				    if($row->stockpile_contract_id != '' || $row->vendor_id != '' || $row->freight_id != '' || $row->labor_id != '' || $row->sales_di != '' || $row->vendor_handling_id != ''){
            ...
            <?php
                $total = $total + $dppTotalPrice;
				$qtyCurah = $qtyCurah + $rowDetail->quantity;
            }
            ?>
        </tbody>
            ...
            <tr>
                ...
                <td><div style="text-align: right;"><?php echo number_format($qtyCurah, 3, ".", ","); ?></div></td>
            </tr>
              <?php
            ...
            <tr>
                <td colspan="7" style="text-align: right;">Total Quantity</td>
                <td><div style="text-align: right;"><?php echo number_format($rowFQ->freight_quantity, 3, ".", ","); ?></div></td>
            </tr>
                <?php
            ...
            <tr>
                <td colspan="7" style="text-align: right;">Total Quantity</td>
                <td><div style="text-align: right;"><?php echo number_format($rowFQ->handling_quantity, 3, ".", ","); ?></div></td>
            </tr>
                <?php
            ...
            <tr>
                <td colspan="7" style="text-align: right;">Total Quantity</td>
                <td><div style="text-align: right;"><?php echo number_format($rowUQ->unloading_quantity, 3, ".", ","); ?></div></td>
            </tr>
                <?php
			...
			if($row->payment_status == 1 && ($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != '')){
				$ppn_amount = 0;
				$pph_amount_converted = 0;
				
			}elseif($row->invoice_id != ''){
				$ppn_amount = $ppnDetail;
				$pph_amount_converted = $pphDetail;
				$total = $total + $ppn_amount - $pph_amount_converted;
			
			}
			else{
				$ppn_amount = $row->ppn_amount;
				$pph_amount_converted = $row->pph_amount_converted;
			}
                ?>
            <tr>
                <td colspan="7" style="text-align: right;">PPN</td>
                <td><div style="text-align: right;"><?php echo number_format($ppn_amount, 2, ".", ","); ?></div></td>
            </tr>
                <?php
            
            
            if($row->payment_status == 1 && $row->pph_amount_converted > 0 && $row->pph_tax_id == 21 && ($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != '')) {
                $total = 0 ;
			} elseif($row->payment_status == 1 && $row->pph_amount_converted > 0 && ($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != '')) {
                $total = 0 ;
			}elseif($row->payment_status == 0 && $row->pph_amount_converted > 0 && $row->pph_tax_id == 21) {
                $total = $total + $row->pph_amount_converted;
			}elseif($row->payment_status == 0 && $row->pph_amount_converted > 0 ) {
                $total = $total - $row->pph_amount_converted;
			}
                ?>
            <tr>
                <td colspan="7" style="text-align: right;">PPh</td>
                <td><div style="text-align: right;">(<?php echo number_format($pph_amount_converted, 2, ".", ","); ?>)</div></td>
            </tr>
             <?php
            if($downPayment > 0) {
                ...
                ?>
            <tr>
                <td colspan="7" style="text-align: right;">Down Payment</td>
...
<?php
$date_now = date('Y-m');
if($row->payment_status == 0 && $date_now <= $edit_date) {
?>
<button class="btn btn-warning" id="returnPayment">Return</button>
<?php
}
?>
