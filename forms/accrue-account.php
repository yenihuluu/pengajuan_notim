...
$sql = "SELECT gl.*, 
CASE WHEN gl.description = 'Cancel Journal' THEN CONCAT(id.generate_code_detail, '-RET') ELSE id.generate_code_detail END AS generateCodeDetail,
...
LEFT JOIN accrue_prediction_detail  id ON id.prediction_detail_id = gl.`accrue_id`
WHERE  gl.amount > 0 AND (general_ledger_module = 'JURNAL ACCRUE') AND gl.accrue_id IN ({$accrueId}) ";
...
<div class="row-fluid">   
    ...
    <thead>
        <tr>
            ...
            <th style="width: 40%;">Generate Code</th>
...
if($rowData->general_ledger_type == 2 && $rowData->general_ledger_module == 'JURNAL ACCRUE' && $rowData->general_ledger_for == 15 && $rowData->accrue_id != 0){
$credit_amount = $rowData->amount;
$status = '';
}

if($rowData->general_ledger_type == 1 && $rowData->general_ledger_module == 'JURNAL ACCRUE' && $rowData->general_ledger_for == 15 && $rowData->accrue_id != 0){
...
$status = '';
        ...
        <tr>
            ...
            <td style="width: 40%;"><?php echo $rowData->generateCodeDetail; ?></td>
