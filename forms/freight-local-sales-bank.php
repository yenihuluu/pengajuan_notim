...
if(isset($_POST['fBankId']) && $_POST['fBankId'] != '') {
    $fBankId = $_POST['fBankId'];

    // <editor-fold defaultstate="collapsed" desc="Query for Freight Cost Data">

    $sql = "SELECT * FROM freight_local_sales_bank WHERE f_bank_id = {$fBankId}
            ";
    $resultData = $myDatabase->query($sql, MYSQLI_STORE_RESULT);

    if($resultData !== false && $resultData->num_rows > 0) {
        $rowData = $resultData->fetch_object();
		$fBankId = $rowData->f_bank_id;
...
<input type="hidden" id="fBankId" name="fBankId" value="<?php echo $fBankId; ?>">
<div class="row-fluid">
