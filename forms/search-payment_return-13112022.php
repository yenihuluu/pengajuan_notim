        ...
        <tbody>
            ...
            if($row->stockpile_contract_id != '') {
				...
				if($row->payment_method == 1){
                    $sql = "SELECT i.invoice_no, id.invoice_detail_id, id.invoice_id, c.po_no,
                            ...
                            LEFT JOIN invoice_detail id  ON id.invoice_id = tpr.invoice_id
                ...
                if($row->payment_method == 1){
                    $sqlInv = "SELECT inv_notim_id FROM payment WHERE payment_id = {$row->payment_id} limit 1";
                    $resultInv = $myDatabase->query($sqlInv, MYSQLI_STORE_RESULT);
                    if($resultInv !== false && $resultInv->num_rows > 0) {
                        $rowInv = $resultInv->fetch_object();
                        $sqlDP = "SELECT sum(settle_amount) as amount FROM pengajuan_payment_dp WHERE inv_notim_id = {$rowInv->inv_notim_id}";
                ...
                $sql = "SELECT hsw.amt_claim as hsw_amt_claim, CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, t.freight_quantity AS quantity, fc.price,
                ...
                ts.`trx_shrink_claim`, COALESCE(ts.amt_claim,0)  AS amountClaim,
            ...
            <tr>
				...
				if($rowDetail->original_amount != '') {
						...
						//if($rowDetail->hsw_amt_claim != '' && $rowDetail->hsw_amt_claim > 0){
						if($rowDetail->hsw_amt_claim != ''){
						...
						$ppnShrink = ($rowDetail->amountClaim + $addShrink ) * ($rowDetail->ppn_tax_value / 100);
						}
						...
						if($rowDetail->pph_tax_category == 1) {
								...
								$pphShrink = ($amountClaim + $addShrink )* ($rowDetail->pph_tax_value / 100);
			...
			<?php }else{?>
				...
				<td>
				<?php
					//if($row->freight_id != '' && $row->payment_method == 1) {
					if($row->freight_id != '') {
						$addShrink =0;
						//if($rowDetail->hsw_amt_claim != '' && $rowDetail->hsw_amt_claim > 0){
						if($rowDetail->hsw_amt_claim != ''){
							$addShrink = $rowDetail->hsw_amt_claim;
							echo number_format($addShrink, 2, ".", ",");
						}else{
							$addShrink - 0;
							echo '-';
						}
				...
				<td>
					...
					$amtTotal = $dppTotal - ($amountClaim + $addShrink);
					//Eva
					//$amtTotal = $dppTotal - $amountClaim + $pphDetail;
					//===
			...
			if($row->vendor_id != '' || $row->freight_id != '' || $row->vendor_handling_id != '' || $row->labor_id != ''){
				...
				$shrink = $shrink + $amountClaim + $addShrink;
				if($row->freight_id != ''){
					//$ppnAmount = $ppnDpp;
					//$pphAmount = $pphDpp;
					//$total = (($dppTotal - $shrink) + $ppnAmount) - $pphAmount;
           ...
           <?php
            ...
            if($row->freight_id != '') {
                ...
                ?>
            <tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>14 <?php }?>" style="text-align: right;">Total Quantity</td>
           ...
           /* if($row->ppn_amount > 0) {
			...
			if($row->payment_status == 1 && ($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != '')){
                ...
                ?>
			<tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Total DPP</td>
                <td><div style="text-align: right;"><?php echo number_format($dpp, 2, ".", ","); ?></div></td>
            </tr>
			<?php if($row->freight_id != '' || $row->labor_id != ''){?>
			<tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Total Susut</td>
                <td><div style="text-align: right;">(<?php echo number_format($shrink, 2, ".", ","); ?>)</div></td>
            </tr>
			<?php }?>
            <tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Total PPN</td>
            ...
            /*if($row->pph_amount > 0 && $row->pph_tax_id == 21) {
                ...
                ?>
            <tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Total PPh</td>
            ...
            if($downPayment > 0) {
                ...
                ?>
            <tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Down Payment</td>
            ...
            <tr>
                <td colspan="<?php if($row->freight_id != '') { ?>12 <?php }else{ ?>13 <?php }?>" style="text-align: right;">Total</td>
                <td><div style="text-align: right;"><?php echo number_format($total, 2, ".", ","); ?></div></td>
            </tr>
...
<input type="hidden" name="action" id="action" value="return_payment1" />
