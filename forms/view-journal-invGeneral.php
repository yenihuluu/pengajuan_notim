...
if($status == 2){
    $glModule = " general_ledger_module = 'RETURN INVOICE'";
}else {
    $glModule = " gl.amount > 0 AND general_ledger_module = 'INVOICE DETAIL'";
}

$sqlInv = "SELECT * FROM `invoice_detail` WHERE invoice_id = {$invId}";
...
if ($resultInv !== false && $resultInv->num_rows > 0) {
    ...
    while ($rowInv = $resultInv->fetch_object()) {
        if($tempInvd == ''){
            $tempInvd = $rowInv->invoice_detail_id;
        }else{
            $tempInvd = $tempInvd .' , '.$rowInv->invoice_detail_id;
        }
    }
}
...
WHERE {$glModule} AND gl.invoice_id in ({$tempInvd})";
...
<div class="row-fluid">   
    ...
    <tbody>
        ...
        if($resultData !== false && $resultData->num_rows > 0) {
			...
			}else if($rowData->general_ledger_module == 'RETURN INVOICE'){
				$debit_amount = $rowData->amount;					
				$credit_amount = $rowData->amount;
			}
