...
switch ($_POST['action']) {
    ...
    break;
    case "validationGrandTotal":
        validationGrandTotal($_POST['checkedSlips'], $_POST['checkedSlipsDP'], $_POST['paymentFor']);
    break;
}
...
function validationGrandTotal($checkedSlips, $checkedSlipsDP, $paymentFor) {
    ...
    $slipNos = $checkedSlips;
    $paymentIds = $checkedSlipsDP;
    ...
    $sqlz = "SELECT  sc.stockpile_id, f.freight_rule, fc.freight_id, con.po_no, 
            ...
            CASE WHEN t.fc_payment_status = 2 THEN (SELECT total_amount FROM payment_ob WHERE transaction_id = t.transaction_id AND payment_method = 3 AND STATUS = 0 ORDER BY payment_ob_id DESC LIMIT 1) ELSE 0 END AS split_ob,

            CASE WHEN t.fc_payment_status = 2 THEN (SELECT MAX(total_amount) FROM payment_handling WHERE transaction_id = t.transaction_id AND payment_method = 3 AND (STATUS != 2 || STATUS != 4)) ELSE 0 END AS split_hc,
            ...
            CASE WHEN t.fc_payment_status = 2 THEN (SELECT MAX(total_amount) FROM payment_curah WHERE transaction_id = t.transaction_id AND payment_method = 3 AND (STATUS != 2 || STATUS != 4)) ELSE 0 END AS split_curah,
            CASE WHEN t.fc_payment_status = 2 THEN  (SELECT max(qty) FROM payment_curah WHERE transaction_id = t.transaction_id and payment_method = 3 and status = 0) ELSE 0 END AS qty_curah,
        ...
        if($resultz !== false && $resultz->num_rows > 0) {
            ...
            while($rowz = $resultz->fetch_object()) {  
                ...
                if($paymentFor == 2){ //FREIGHT
                    ...
                    $dppAddShrinkPrice = $rowz->hsw_amt_claim;
                    $freightPrice = 0;
                    ...
                    if($tempAvailableDp > 0){ //1250000
                        if ($tempAvailableDp > $dppPrice_dppShrink){ //1250000 > 4193800
                                ...
                                $splitAmount = 0;
                                $slipAmount = $dppPrice_dppShrink - $tempAvailableDp;
                                if($slipAmount < 0){
                                    $splitAmount = 1;
                                }                                    
                            }
...
function updatePengajuanPayment($idPP, $paymentFor) {
   ...
   // echo $sql;
    ...
    if ($result->num_rows > 0 && $count > 0) {
        ...
        $update = "UPDATE pengajuan_payment SET total_qty = $totalQty, total_shrink = $totalShrink, total_dpp = $totalDpp, "
...
//echo $sql;
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            // echo $unitPrice;
            if ($row->pph_tax_id == 0) {
                $dppTotalPrice = round(($unitPrice * $curahQty),2);
            } else {
                if ($row->pph_tax_category == 1) {
                    $dppTotalPrice = ($unitPrice * $curahQty) / ((100 - $row->pph_tax_value) / 100);
                } else {
                    $dppTotalPrice = ($unitPrice * $curahQty);
                }
            }
            $returnValue .= '<tr>';
            if($idPP == ''){
           ...
           // echo $sqlAA;
            ...
            if($DpCount > 0){ //settle != 2 => 
                ...
                if($settle == 1) {
                    ...
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSettle" id="checkedSettle" value="'. $row->payment_id .'" onclick="checkSettle_Curah('. $stockpileId .', '. $vendorId .', '. "'".$settle ."'" .', '. $contractIds .',\'NONE\', \'NONE\', \''. $paymentFromCur .'\', \''. $paymentToCur .'\', '. $idPP .');"  /></td>';                    
                }
...
function setSlipFreight_1($stockpileId_1, $freightId_1, $contractFreight, $checkedSlips, $ppn, $pph, $paymentFromFP, $paymentToFP, $idPP)
{
    ...
    //start edit by eva
    $sql = "SELECT COALESCE(hsw.amt_claim) as hsw_amt_claim, COALESCE(ts.amt_claim,0) AS amt_claim, 
                {$selectPaymentOA} sc.stockpile_id, f.freight_rule, txpph.tax_id AS pph_tax_id, 
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
                ...
                if($idPP == ''){
                    if ($checkedSlips != '') {
                        ...
                        if ($pos === false) {
                            ...
                            $totalAllShrink = $totalAllShrink + $dppShrinkPrice + $hswAmtClaim;
                            //end add by eva
                    ...
                    $grandTotal = $row->grand_total;
                    //start add by eva
                    $totalAllShrink = $totalAllShrink + ($row->amt_claim + $row->hsw_amt_claim);
                ...
                // Start Add by Eva
                if($hswAmtClaim != '' && $hswAmtClaim > 0){
                    //Additional Shrink
                    $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format($hswAmtClaim, 2, ".", ",") . '</td>';
                }else{
                    $returnValue .= '<td style="text-align: right; width: 20%;">' . '-' . '</td>';
                }
                // End Add by Eva
            ...
            if($boolCheked) {
                    ...
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="13" style="text-align: right;">Total Amount</td>';
        ...
        //$totalPph = ($pphValue/100) * $totalPrice;
            ...
            //start add by eva
            $returnValue .= '<input type="hidden" id="totaldppAddShrink" name="totaldppAddShrink" value="' . round($totaldppAddShrink, 2) . '" />';
            //end add by eva
...
function setSlipFreight_settle($stockpileId, $freightId, $settle, $contractFreight, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFrom, $paymentTo, $idPP) {
    ...
    $sql = "SELECT {$selectPaymentOA} t.*, sc.stockpile_id, fc.freight_id, con.po_no, f.freight_rule,
                f.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value, 
                txpph.tax_id AS pph_tax_id, txpph.tax_value AS pph_tax_value, txpph.tax_category AS pph_tax_category,  
    ...
    // END WHILE
        ...
        if($boolCheked) {
            ...
            $returnValue .= '</tr>';
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="12" style="text-align: right;">Total Shrink</td>';
            $returnValue .= '<td style="text-align: right;">'. number_format($totaldppShrink, 2, ".", ",") .'</td>';
            $returnValue .= '</tr>';
        ...
        //--------------------------------------------------------------------Down Payment------------------------------------------------------------------------------------------------------------------------------------------------            
            if($idPP == ''){ //INPUT DATA
                ...
                if($DpCount > 0){ //settle != 2 
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right; color:red;">Settlement Only ?</td>';
                    if($settle == 1) {
                    ...
                    $returnValue .= '<td colspan="2" style="text-align: right;">Grand Total</td>'; //for settlement 50%
                }else{
                    $returnValue .= '<td colspan="12" style="text-align: right;">Grand Total</td>'; //for settlemetn 100%
                }
            }else{ // VIEW DATA
                $sql = "SELECT ppd.*, CASE WHEN pp.settlement_status = 0 THEN 'No' ELSE 'Yes' END AS settle FROM pengajuan_payment_dp ppd 
                        LEFT JOIN pengajuan_payment pp ON pp.idPP = ppd.idPP WHERE pp.idPP = {$idPP}";
               ...
               // echo $sql;
                ...
                $returnValue .= '<tr>';
                $returnValue .= '<td colspan="10" style="text-align: right; color:red;">Settlement Only ?</td>';
                $returnValue .= '<td colspan="1" style="text-align: right;">'. $tempSettle.'</td>';
                $returnValue .= '<td colspan="1" style="text-align: right;">Grand Total</td>';
            }
...
function setSlipFreight_settleApprove($stockpileId, $freightId, $paymentFrom, $paymentTo, $idPP ) {
    ...
    $sql = "SELECT poa.amount AS settleAmount, poa.qty as quantity1, t.*, sc.stockpile_id, fc.freight_id, con.po_no, f.freight_rule,
                f.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
				...
				WHEN ts.trx_shrink_tolerance_persen > 0 AND ((t.shrink/t.send_weight) * 100 > ts.trx_shrink_tolerance_persen) AND (SELECT transaction_id FROM transaction_shrink_weight WHERE transaction_id = t.transaction_id) IS NOT NULL THEN t.shrink - (SELECT weight_persen FROM transaction_shrink_weight WHERE transaction_id = t.transaction_id)
                ELSE 0 END,10) AS qtyClaim,
                CASE WHEN t.fc_payment_status = 2 THEN (SELECT max(amount) FROM payment_oa WHERE transaction_id = t.transaction_id and payment_method = 3 and (status != 4 || status != 2)) ELSE 0 END AS split_oa
            FROM TRANSACTION t
            LEFT JOIN freight_cost fc
                ON fc.freight_cost_id = t.freight_cost_id
            LEFT JOIN freight f
    ...
    if ($result->num_rows > 0) {
        $sql2 = "SELECT payment_method from pengajuan_payment where idpp = {$idPP}";
        $result2 = $myDatabase->query($sql2, MYSQLI_STORE_RESULT);
        if ($result2->num_rows == 1) {
            $row2 = $result2->fetch_object();
            $paymentMethod = $row2->payment_method;
        }
        $returnValue = '<div class="span12 lightblue">';
		$returnValue .= '<form id = "frm1">';
        $returnValue .= '<input type="hidden" id="pksContract" name="pksContract" value="' . $contractFreights . '" />';  //khusus  kode vendor_ID
		//$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead>
                            <tr>
                                ...
                                <th>Vehicle No</th><th>Quantity</th>
                                <th>Freight Cost / kg</th>
                                <th>Amount</th><th>Shrink Qty Claim</th>
                                <th>Shrink Price Claim</th>
                                <th>Shrink Amount</th>
                                <th>Total Amount</th>
                            </tr>
    ...
    // START WHILE
        while($row = $result->fetch_object()) {
			...
			if($row->freight_rule == 1){
				$fp =  $row->settleAmount;
                $fp1 =  ($row->freight_price * $row->send_weight)- $row->split_oa;
				$fq = $row->send_weight;
			}else{
				$fp =$row->settleAmount;
                $fp1 = ($row->freight_price * $row->quantity)- $row->split_oa;
				$fq = $row->quantity1;
			}
            ...
            // by YENI
            if($row->freight_rule == 1){
                $dpp = $dpp + (($row->freight_price * $row->send_weight) - $row->settleAmount);
                $qty = $qty + $row->send_weight;
            }else{
                $dpp = $dpp + (($row->freight_price * $row->quantity)- $row->settleAmount);
            ...
            //JIKA ADA SUSUT
            if($row->qtyClaim > 0 || $row->trx_shrink_claim > 0){
                $freightPrice1 = 0;
                $fq1 = 0;
                $freightPrice1 = $freightPrice + $dppShrinkPrice;
                $fq1 = $freightPrice1/$row->freight_price;
                $freightPrice = $freightPrice1;
                $fq = $fq1;
                $amountPrice = $freightPrice1 - $dppShrinkPrice;
                $totalPrice = $amountPrice;

                if($row->ppn_tax_id != 0 && $row->ppn_tax_id != '') {
                    $totalPPN = ($amountPrice * ($row->ppn_tax_value / 100));
                }
                if($row->pph_tax_id != 0 && $row->pph_tax_id != '') {
                    $totalPPh = ($amountPrice * ($row->pph_tax_value / 100));
                }
            }
            ...
            $returnValue .= '<td style="width: 10%;">'. $row->vendor_code .'</td>';
            $returnValue .= '<td style="width: 10%;">'. $row->vehicle_no .'</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($fq, 2, ".", ",") .'</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($row->freight_price, 2, ".", ",") .'</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($freightPrice, 2, ".", ",") .'</td>';
    ...
    // END WHILE
        ...
        //SELECTED DP 1
            $returnValue .= '<tfoot>';
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="11" style="text-align: right;">Total</td>';
            $returnValue .= '<td style="text-align: right;">'. number_format($totalPrice, 2, ".", ",") .'</td>';
            $returnValue .= '</tr>';
			
			$returnValue .= '<tr>';
            $returnValue .= '<td colspan="11" style="text-align: right;">PPN</td>';
            $returnValue .= '<td style="text-align: right;">'. number_format($totalPPN, 2, ".", ",") .'</td>';
            $returnValue .= '</tr>';

            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="11" style="text-align: right;">PPh</td>';
            $returnValue .= '<td style="text-align: right;">'. number_format($totalPPh, 2, ".", ",") .'</td>';
            $returnValue .= '</tr>';
           
            // untuk preview Register Invoice
            $sqlInv = "SELECT inv_notim_id, status_payment as status FROM invoice_notim where idpp = {$idPP}";
            $resultInv = $myDatabase->query($sqlInv, MYSQLI_STORE_RESULT);
           // echo $sqlInv;
            if ($resultInv->num_rows == 1){
                $propertyStatus = "and (dp.status in (4,2,1))";
            }else{
                $propertyStatus = "and dp.status in (0,2)";
            }

			$sql = "SELECT sum(dp.settle_amount) as settleAmount, sum(dp.available_amount) as avalilabelAmount, 
                    (SELECT bank_code FROM bank WHERE bank_id = p.bank_id) AS bank_code, 
                    (SELECT bank_type FROM bank WHERE bank_id = p.bank_id) AS bank_type,
                    (SELECT currency_code FROM currency WHERE currency_id = p.currency_id) AS pcur_currency_code,
                    CASE WHEN p.payment_location = 0 THEN 'HOF'
                        ELSE (SELECT stockpile_name FROM stockpile WHERE stockpile_id = p.payment_location) 
                        END AS payment_location,
                    dp.* FROM pengajuan_payment_dp dp 
                    LEFT JOIN payment p ON p.payment_id = dp.payment_id  where dp.idpp = {$idPP} {$propertyStatus}";
            $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
       //    echo $sql;
			...
			if ($result->num_rows > 0) {
                while($row = $result->fetch_object()) {
                    ...
                    $avalilabelAmount = $row->avalilabelAmount;
                    $dpAmount = $settleAmount + $avalilabelAmount;
                  ...
                  //   echo $voucherCode .' # '. $rowData->payment_no; 
                    ...
                    if($paymentMethod == 3){
                        $returnValue .= '<tr>';
                        $returnValue .= '<td colspan="7" style="text-align: right;">Down Payment</td>';
                        $returnValue .= '<td colspan="2" style="text-align: right;">'. $voucherCode .' # '. $row->payment_no.'</td>';
                        $returnValue .= '<td  style="text-align: right;">'. $row->po_no.'</td>';
                        $returnValue .= '<td  style="text-align: right;">'. $row->contract_no.'</td>';
                        $returnValue .= '<td style="text-align: right;">'. number_format($dpAmount, 2, ".", ",") .'</td>';
                        $returnValue .= '</tr>';

                        $downPayment = $downPayment + $dpAmount;
                    }
            ...
            $sqlPP = "SELECT ppn_amount, pph_amount, settlement_status FROM pengajuan_payment where idPP = {$idPP}";
            $result = $myDatabase->query($sqlPP, MYSQLI_STORE_RESULT);
            $row = $result->fetch_object();
            
            if($paymentMethod == 3){
                $returnValue .= '<tr>';
                $returnValue .= '<td colspan="8" style="text-align: right;">Settlement Only ?</td>';
                if($row->settlement_status == 0  ){
                    if($grandTotal < 0){
                        $grandTotal = 0;
                    }
                    $returnValue .= '<td style="text-align: right;">No</td>';
                }else{
                    $returnValue .= '<td style="text-align: right;">Yes</td>';
                    $grandTotal = 0;
                }
                ...
                $returnValue .= '<td colspan="11" style="text-align: right;">Grand Total</td>';
                $returnValue .= '<td style="text-align: right;">'. number_format($grandTotal, 2, ".", ",") .'</td>';
                $returnValue .= '</tr>';
            }
		...
		$returnValue .= '<input type="hidden" id="ppn" name="ppn" value="'. $totalPPN .'" />';
        $returnValue .= '<input type="hidden" id="pph" name="pph" value="'. $totalPPh .'" />';
        $return_val .= '<input type="hidden" value="|' . number_format($grandTotal, 2, ".", ",") . '|" />';  
        $returnValue .= '<input type="hidden" id="dpp" name="dpp" value="'. round($dpp, 2) .'" />';
...
function setSlipFreight_approve($stockpileId, $freightId, $paymentFrom, $paymentTo, $idPP)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
...
function setSlipHandling_settleApprove($stockpileId, $vendorHandlingId, $paymentFrom, $paymentTo, $idPP ) {
    global $myDatabase;
    $returnValue = '';
	$whereProperty = '';
    $return_val = '';

    $sql = "SELECT poh.amount as settleAmount, t.*, sc.stockpile_id, poh.qty as quantity1, vhc.vendor_handling_id, con.po_no, vh.vendor_handling_rule,
            vh.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
            txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value, v.vendor_code,
            CASE WHEN t.fc_payment_status = 2 THEN (SELECT MAX(amount) FROM payment_handling WHERE transaction_id = t.transaction_id and payment_method = 3 and (status != 4 || status != 2)) ELSE 0 END AS split_handling
            FROM TRANSACTION t
            ...
            LEFT JOIN contract con
                ON con.contract_id = sc.contract_id
            INNER JOIN payment_handling poh 
                ON poh.transaction_id = t.transaction_id
            WHERE vhc.vendor_handling_id = {$vendorHandlingId}
			AND sc.stockpile_id = {$stockpileId}
			AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
            AND t.company_id = {$_SESSION['companyId']}
			AND freight_price != 0 and t.adj_oa IS NULL
			AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFrom', '%d/%m/%Y') AND STR_TO_DATE('$paymentTo', '%d/%m/%Y')
            AND poh.idPP = {$idPP}
			ORDER BY t.slip_no ASC";
            //echo $sql;
    ...
    // START WHILE
        while($row = $result->fetch_object()) {
			...
			$qtyh = ceil(($row->settleAmount/$row->handling_price));
           ...
           // $amountPrice = $dppTotalPrice;
            ...
            $returnValue .= '<td style="width: 15%;">'. $row->po_no .'</td>';
            $returnValue .= '<td style="width: 10%;">'. $row->vendor_code .'</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($qtyh, 0, ".", ",") .'</td>';
    ...
    // END WHILE
        ...
        //SELECTED DP 1
			...
			// $sql = "SELECT * FROM pengajuan_payment_dp where idpp = {$idPP} and status = 0";
            // $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
            ...
            // echo $sqlInv;
            if ($resultInv->num_rows == 1){
                $propertyStatus = "and (dp.status = 4 || dp.status = 2 || dp.status = 1)";
            }else{
                $propertyStatus = "and dp.status = 0";
            }
...
function setSlipHandling_settle($stockpileId, $vendorHandlingId, $settle, $contractHandling, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromHP, $paymentToHP, $idPP) {
  ...
  //  echo "SS " . $settle;
    ...
    if($idPP != ''){ 
        ...
        $whereProperty = "   AND  t.hc_payment_id IS NULL
                            AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
                            AND t.company_id = {$_SESSION['companyId']}
                            AND t.fc_payment_status <> 1 AND vhc.price != 0 AND t.handling_quantity != 0
                            AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFromHP', '%d/%m/%Y') AND STR_TO_DATE('$paymentToHP', '%d/%m/%Y')";
        // $wherePengajuan = " LEFT JOIN payment_ pao ON pao.transaction_id = t.transaction_id AND pao.status = 0";
    }

    $sql = "SELECT {$selectPaymentH} t.*, sc.stockpile_id, vhc.vendor_handling_id, con.po_no, vh.vendor_handling_rule,
                vh.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value, v.vendor_code,
        ...
        if($idPP == ''){
            ...
            if($DpCount > 0){ //settle != 2 => 
                ...
                if($settle == 1) {
                    ...
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSettle" id="checkedSettle" value="'. $row->payment_id .'" onclick="checkSettle_handling('. $stockpileId .', '. $vendorHandlingId .', '. "'".$settle ."'" .', '. $contractIds .',\'NONE\', \'NONE\', \''. $paymentFromHP .'\', \''. $paymentToHP .'\', '. $idPP .');"  /></td>';                    
                }
                $returnValue .= '<td colspan="1" style="text-align: right;">Grand Total</td>'; //for settlement 50%
            }else{
                $returnValue .= '<td colspan="1" style="text-align: right;">Grand Total</td>'; //for settlemetn 100%
...
function setSlipHandling($stockpileId, $vendorHandlingId, $contractHandling, $checkedSlips, $ppn, $pph, $paymentFromHP, $paymentToHP, $idPP)
{
    ...
    if($idPP != ''){ 
        ...
        $whereProperty = " AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
                            ...
                            AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFromHP', '%d/%m/%Y') AND STR_TO_DATE('$paymentToHP', '%d/%m/%Y')
                            AND  t.hc_payment_id IS NULL AND t.fc_payment_status <> 1";
        // $wherePengajuan = " LEFT JOIN payment_ pao ON pao.transaction_id = t.transaction_id AND pao.status = 0";
    }
...
function setSlipUnloading_settle($stockpileId, $laborId, $settle, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromUP, $paymentToUP, $idPP) {
    ...
    if($idPP != ''){ 
        ...
        $whereProperty = "AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
                            AND t.company_id = {$_SESSION['companyId']}
                            AND t.uc_payment_id IS NULL AND t.unloading_cost_id IS NOT NULL AND t.adj_ob IS NULL AND t.fc_payment_status <> 1
                            AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFromUP', '%d/%m/%Y') AND STR_TO_DATE('$paymentToUP', '%d/%m/%Y')";
        // $wherePengajuan = " LEFT JOIN payment_ pao ON pao.transaction_id = t.transaction_id AND pao.status = 0";
    }

    $sql = "SELECT {$selectPaymentU} t.*,
            ...
            if($DpCount > 0){ 
                ...
                $returnValue .= '<td colspan="4" style="text-align: right;">Settlement Only ?</td>';
                    if($settle == 1) {
                        ...
                        $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSettle" id="checkedSettle" value="'. $row->payment_id .'" onclick="checkSettle_unloading('. $stockpileId .', '. $laborId .', '. "'".$settle ."'" .',\'NONE\', \'NONE\', \''. $paymentFromUP .'\', \''. $paymentToUP .'\','. $idPP .');"  /></td>';                    
                    }        
                $returnValue .= '<td colspan="1" style="text-align: right;">Grand Total</td>'; //for settlement 50%
            }else{
                $returnValue .= '<td colspan="7" style="text-align: right;">Grand Total</td>'; //for settlemetn 100%
            }
        }else{ // VIEW DATA
            $sql = "SELECT ppd.*, CASE WHEN pp.settlement_status = 0 THEN 'No' ELSE 'Yes' END AS settle FROM pengajuan_payment_dp ppd 
                    LEFT JOIN pengajuan_payment pp ON pp.idPP = ppd.idPP WHERE pp.idPP = {$idPP}";
...
function setSlipUnloading($stockpileId, $laborId, $checkedSlips, $ppn, $pph, $paymentFromUP, $paymentToUP, $idPP)
{
    ...
    if($idPP != ''){ 
        ...
        $whereProperty = "AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
                            AND t.company_id = {$_SESSION['companyId']}
                            AND t.unloading_cost_id IS NOT NULL AND t.fc_payment_status <> 1
                            AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFromUP', '%d/%m/%Y') AND STR_TO_DATE('$paymentToUP', '%d/%m/%Y')
                            AND t.uc_payment_id IS NULL AND t.fc_payment_status <> 1";
        // $wherePengajuan = " LEFT JOIN payment_ pao ON pao.transaction_id = t.transaction_id AND pao.status = 0";
    }

    $sql = "SELECT {$selectPaymentU} t.*, l.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value,
...
function setSlipUnloading_settleApprove($stockpileId, $laborId, $paymentFromUP, $paymentToUP, $idPP)
{
    ...
    $returnValue = '';
        ...
        $sql = "SELECT pob.amount as settleAmount, t.*,l.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                            txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value,
                            CASE WHEN t.fc_payment_status = 2 THEN (SELECT MAX(amount) FROM payment_ob WHERE transaction_id = t.transaction_id and payment_method = 3 and (status != 4 || status != 2)) ELSE 0 END AS split_ob
                        FROM `transaction` t
                        INNER JOIN labor l
                            ON l.labor_id = t.labor_id
                        LEFT JOIN tax txppn
                    ...
                    ON pob.transaction_id = t.transaction_id
                        WHERE t.labor_id ={$laborId}
                        AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId})
                        AND t.company_id = {$_SESSION['companyId']}
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            $dppTotalPrice = 0;
            if ($row->pph_tax_id == 0 || $row->pph_tax_id == '') {
                $dppTotalPrice = $row->settleAmount;
            } else {
                if ($row->pph_tax_category == 1) {
                    $dppTotalPrice = $row->settleAmount / ((100 - $row->pph_tax_value) / 100);
                } else {
                    $dppTotalPrice = $row->settleAmount;
                }
            }
            ...
            $returnValue .= '</tr>';
             ...
             $sql = "SELECT sum(dp.settle_amount) as settleAmount, sum(dp.available_amount) as avalilabelAmount, 
                    ...
                    CASE WHEN p.payment_location = 0 THEN 'HOF'
                        ...
                        END AS payment_location,
                    dp.* 
                    FROM pengajuan_payment_dp dp 
                    LEFT JOIN payment p ON p.payment_id = dp.payment_id 
                    where dp.idpp = {$idPP} {$propertyStatus}";
             $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
          //   echo $sql;
        ...
        $returnValue .= '</table>';
        $returnValue .= '<input type="hidden" id="totalPrice" name="totalPrice" value="' . round($totalPrice, 2) . '" />';
        $returnValue .= '<input type="hidden" id="totalPrice2" name="totalPrice2" value="'. round($totalPrice, 2) .'" />';
		$returnValue .= '<input type="hidden" id="ppn" name="ppn" value="'. $totalPPN .'" />';
        $returnValue .= '<input type="hidden" id="pph" name="pph" value="'. $totalPPh .'" />';
        $returnValue .= '<input type="hidden" id="downPayment" name="downPayment" value="' . $downPayment . '" />';
        $returnValue .= '<input type="hidden" id="grandTotal" name="grandTotal" value="' . round($grandTotal, 2) . '" />';
