...
$sql = "SELECT CASE WHEN pp.vendor_id IS NOT NULL THEN ven.vendor_name
        ...
        LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = pp.stockpile_contract_id
        ...
        WHERE ust.user_id = {$_SESSION['userId']} and pp.dp_status = 0 and pp.idPP IN ({$selectedCheck}) ORDER BY idPP DESC";  
//echo $sql;
...
if($result->num_rows > 0){
    while($row = $result->fetch_object()){
     ...
     //  $objPHPExcel->getActiveSheet()->getCell("C{$rowActive}")->setValueExplicit($row->vendorName, PHPExcel_Cell_DataType::TYPE_STRING);
       ...
       $objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $row->qty); //tidak tau 
       $objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $row->price);
       $objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $row->dpp);
       $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->ppn_amount);
       $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", $row->pph_amount);
       $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", $row->amount);
      // $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", $row->remaks);
