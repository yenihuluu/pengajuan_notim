...
LEFT JOIN customer c ON c.`customer_id` = b.`vendor_id` WHERE b.`freight_cost_id` = t.freight_cost_id) END AS freight_code, f.freight_id, f.freight_rule,
		 ...
		 fp.payment_no AS fPayment, up.payment_no AS uPayment, hp.payment_no AS hPayment,ts.`trx_shrink_claim` AS shrink_claim, 
                ...
                fc.contract_pkhoa, l.labor_name, cpd.contract_pks_detail_id, vc.vendor_curah_name as vendor_curah, tas.qty_add_shrink
...
$objPHPExcel->getActiveSheet()->setCellValue("AP{$rowActive}", "Shipment Code");
$objPHPExcel->getActiveSheet()->setCellValue("AQ{$rowActive}", "Freight Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AR{$rowActive}", "Unloading Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AS{$rowActive}", "Handling Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AT{$rowActive}", "Vendor Bongkar");
$objPHPExcel->getActiveSheet()->setCellValue("AU{$rowActive}", "Sumber PKS");



$objPHPExcel->getActiveSheet()->setCellValue("AV{$rowActive}", "Labor Rules Price");
...
//    $objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $row->vehicle_no);
   ...
   // $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $row->freight_code);
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AP{$rowActive}", $row->shipment_no2, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AQ{$rowActive}", $row->fPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AR{$rowActive}", $row->uPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AS{$rowActive}", $row->hPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AT{$rowActive}", $row->labor_name, PHPExcel_Cell_DataType::TYPE_STRING);
	if($row->contract_pks_detail_id != '') {
        $objPHPExcel->getActiveSheet()->setCellValueExplicit("AU{$rowActive}", $row->vendor_curah, PHPExcel_Cell_DataType::TYPE_STRING);
    } else {
	    $objPHPExcel->getActiveSheet()->setCellValueExplicit("AU{$rowActive}", $row->vendor_name, PHPExcel_Cell_DataType::TYPE_STRING);
    }
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("AV{$rowActive}", $row->laborRules);
