...
$sql = "SELECT t.*,
		 ...
		 fp.payment_no AS fPayment, up.payment_no AS uPayment, hp.payment_no AS hPayment,ts.`trx_shrink_claim` AS shrink_claim, 
                ...
                CASE WHEN t.rsb = 1 AND t.ggl = 0 THEN 'RSB'
                    WHEN t.rsb = 1 AND t.ggl = 1 THEN 'RSB + GGL'
                    WHEN t.rsb = 0 AND t.ggl = 1 THEN 'GGL'
                    WHEN t.rsb = 0 AND t.ggl = 0 THEN 'Uncertified'
                ELSE NULL END
            WHEN t.transaction_type = 2 THEN
                CASE WHEN t.rsb = 1 THEN 'RSB'
                    WHEN t.ggl = 1 THEN 'GGL'
                    WHEN t.rsb_ggl = 1 THEN 'RSB + GGL'
                    WHEN t.rsb = 0 AND t.ggl = 0 AND t.rsb_ggl = 0 THEN 'Uncertified'
                ELSE NULL END
            ELSE NULL END AS certification
...
$objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "Sertifikat");

$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "Type");
$objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", "Berat Kirim");
$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", "Berat Bruto");
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "Berat Tarra");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "Berat Netto");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", "Susut");
...
$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", "Biaya Angkut (/Kg)");
$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", "Biaya Angkut");
$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", "Klaim Susut");
$objPHPExcel->getActiveSheet()->setCellValue("AB{$rowActive}", "Total Biaya Angkut");
$objPHPExcel->getActiveSheet()->setCellValue("AC{$rowActive}", "Biaya Bongkar");
$objPHPExcel->getActiveSheet()->setCellValue("AD{$rowActive}", "Vendor Handling");
$objPHPExcel->getActiveSheet()->setCellValue("AE{$rowActive}", "Biaya Handling");
$objPHPExcel->getActiveSheet()->setCellValue("AF{$rowActive}", "Biaya Handling (/Kg)");
$objPHPExcel->getActiveSheet()->setCellValue("AG{$rowActive}", "R");
$objPHPExcel->getActiveSheet()->setCellValue("AH{$rowActive}", "R (Tanggal)");
$objPHPExcel->getActiveSheet()->setCellValue("AI{$rowActive}", "Slip Reference");
$objPHPExcel->getActiveSheet()->setCellValue("AJ{$rowActive}", "Type");
$objPHPExcel->getActiveSheet()->setCellValue("AK{$rowActive}", "Inventory");

$objPHPExcel->getActiveSheet()->setCellValue("AL{$rowActive}", "Balance (Q)");
$objPHPExcel->getActiveSheet()->setCellValue("AM{$rowActive}", "Shipment Code");
$objPHPExcel->getActiveSheet()->setCellValue("AN{$rowActive}", "Freight Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AO{$rowActive}", "Unloading Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AP{$rowActive}", "Handling Payment");
$objPHPExcel->getActiveSheet()->setCellValue("AQ{$rowActive}", "Vendor Bongkar");
$objPHPExcel->getActiveSheet()->setCellValue("AR{$rowActive}", "Sumber PKS");
...
//    $objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $row->vehicle_no);
   ...
   // $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $row->freight_code);
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("M{$rowActive}", $row->contract_no, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("N{$rowActive}", $row->certification, PHPExcel_Cell_DataType::TYPE_STRING);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $row->send_weight);
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $row->bruto_weight);
    $objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", $row->tarra_weight);
    $objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", $row->netto_weight);
	$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $shrink);
	$objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", $pks_price);
	$objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", $pks_price * $row->quantity);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", $row->freight_price);
	$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", $fcTotal);
	$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", $fc_shrink);
	$objPHPExcel->getActiveSheet()->setCellValue("AB{$rowActive}", $fc_total);
	
    $objPHPExcel->getActiveSheet()->setCellValue("AC{$rowActive}", $uc_total);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AD{$rowActive}", $row->vendor_handling_name, PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->setCellValue("AE{$rowActive}", $hc_total);
	$objPHPExcel->getActiveSheet()->setCellValue("AF{$rowActive}", $row->handling_price);
    $objPHPExcel->getActiveSheet()->getStyle("AG{$rowActive}:AI{$rowActive}")->applyFromArray($styleArray9);
    $objPHPExcel->getActiveSheet()->setCellValue("AG{$rowActive}", $row->user_name);
	$objPHPExcel->getActiveSheet()->setCellValue("AH{$rowActive}", $row->modify_date);
    $objPHPExcel->getActiveSheet()->setCellValue("AI{$rowActive}", $row->slip_retur);
    $objPHPExcel->getActiveSheet()->setCellValue("AJ{$rowActive}", $row->contract_type2);
    $objPHPExcel->getActiveSheet()->setCellValue("AK{$rowActive}", $quantity);
    
    $objPHPExcel->getActiveSheet()->setCellValue("AL{$rowActive}", $balanceQuantity);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AM{$rowActive}", $row->shipment_no2, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AN{$rowActive}", $row->fPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AO{$rowActive}", $row->uPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AP{$rowActive}", $row->hPayment, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AQ{$rowActive}", $row->labor_name, PHPExcel_Cell_DataType::TYPE_STRING);
	if($row->contract_pks_detail_id != '') {
        $objPHPExcel->getActiveSheet()->setCellValueExplicit("AR{$rowActive}", $row->vendor_curah, PHPExcel_Cell_DataType::TYPE_STRING);
    } else {
	    $objPHPExcel->getActiveSheet()->setCellValueExplicit("AR{$rowActive}", $row->vendor_name, PHPExcel_Cell_DataType::TYPE_STRING);
    }
...
// Set column width
for ($temp = ord("A"); $temp <= ord("AG"); $temp++) {
...
if ($bodyRowEnd > $headerRow) {
	...
	$objPHPExcel->getActiveSheet()->getStyle("AH" . ($headerRow + 1) . ":AH{$bodyRowEnd}")->getNumberFormat()->setFormatCode("DD-MMM-YYYY");
}
...
// Set number format for Amount 
$objPHPExcel->getActiveSheet()->getStyle("P" . ($headerRow + 1) . ":V{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
$objPHPExcel->getActiveSheet()->getStyle("Y" . ($headerRow + 1) . ":AC{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
$objPHPExcel->getActiveSheet()->getStyle("AE" . ($headerRow + 1) . ":AF{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
$objPHPExcel->getActiveSheet()->getStyle("AK" . ($headerRow + 1) . ":AL{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
