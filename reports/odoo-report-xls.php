...
if ($periodFrom != '' && $periodTo != '') {
    $whereProperty .= " AND (CASE WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 AND gl.general_ledger_module = 'CONTRACT ADJUSTMENT' THEN DATE_FORMAT((SELECT adjustment_date FROM contract WHERE contract_id = gl.contract_id), '%Y-%m-%d')
		...
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_for = 10 AND gl.general_ledger_module = 'RETURN INVOICE' THEN (SELECT DATE_FORMAT(i.input_date, '%Y-%m-%d') FROM invoice i LEFT JOIN invoice_detail id ON id.invoice_id = i.invoice_id WHERE id.invoice_detail_id = gl.invoice_id)
...
if ($stockpileId != '') {
    ...
    $whereProperty .= " AND (CASE WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 THEN (SELECT GROUP_CONCAT(s.stockpile_name) FROM stockpile_contract sc INNER JOIN stockpile s ON s.stockpile_id = sc.stockpile_id WHERE sc.contract_id = gl.contract_id AND sc.quantity > 0 GROUP BY sc.contract_id)
...
(SELECT `account_name` FROM  account a WHERE a.account_id = gl.account_id) AS account_name,
	...
	CASE WHEN gl.payment_id IS NOT NULL AND (gl.general_ledger_for = 5 OR gl.general_ledger_for = 8) THEN  (SELECT shipment_no FROM shipment WHERE shipment_id = (SELECT shipment_id FROM payment WHERE payment_id = gl.payment_id)) 
		...
		WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 THEN (SELECT mh.kode_mutasi FROM mutasi_header mh LEFT JOIN mutasi_contract mc ON mh.mutasi_header_id = mc.mutasi_header_id LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = mc.stockpile_contract_id WHERE sc.contract_id = gl.contract_id)  
	...
	CASE WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 AND gl.general_ledger_module = 'CONTRACT ADJUSTMENT' THEN (SELECT adjustment FROM contract WHERE contract_id = gl.contract_id)
	 ...
	 CASE WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 THEN (SELECT price_converted FROM contract WHERE contract_id = gl.contract_id) 
		...
		WHEN gl.payment_id IS NOT NULL AND gl.general_ledger_for = 13 THEN (SELECT GROUP_CONCAT(vhc.price_converted) FROM vendor_handling_cost vhc LEFT JOIN `transaction` t ON vhc.handling_cost_id = t.handling_cost_id WHERE t.transaction_id = gl.t_id)
...
'' AS exchange_rate, '' AS bank_code, '' AS bank_type, '' AS pcur_currency_code, '' AS payment_location2, ru.payment_no AS payment_no, ru.payment_no AS payment_no2, '' AS frieght_id, '' AS vendor_handling_id, '' AS labor_id, '' AS payment_status, '' AS payment_date, '' AS fc_tax_category,
...
$fileName = "Odoo Report Jatim-" . $periodFull . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . $module . ".xls";
$onSheet = 0;
$lastColumn = "Y";
...
$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", "No Journal");
$objPHPExcel->getActiveSheet()->setCellValue("C{$rowActive}", "Journal");
$objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", "Account");
$objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", "Partner");
$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", "Partner Name");
...
$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", "PO No");
$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", "Contract No");
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Slip No");
$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "Invoice No");
$objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "Quantity");
$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "Price");
$objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", "Debit");
$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", "Credit");
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "Method");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "Type");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", "Original Invoice");
$objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", "Tax Invoice");
$objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", "Cheque No");
$objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", "Original Amount");
$objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", "Transaction Currency");
$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", "Kurs");
...
while ($row = $result->fetch_object()) {
	...
	$sqlNew = "INSERT into gl_odoo (general_ledger_id, export_by, export_date) values ('{$glId}', '{$_SESSION['userName']}', STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'))";
...
//==END PAYMENT DEBIT==//
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", $voucherNo);
    $objPHPExcel->getActiveSheet()->setCellValue("C{$rowActive}", $row->general_ledger_module);
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("D{$rowActive}", $row->account_no, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $row->supplier_code);
    $objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $row->supplier_name);
    $objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", $row->remarks);
    $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $stockpileName);
    ...
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("L{$rowActive}", $row->slip_no, PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("M{$rowActive}", $row->invoice_no, PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", $row->quantity);
    $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", $row->price);
    $debitAmount = 0;
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $debitAmount);
    $creditAmount = 0;
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $creditAmount);
    ...
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("T{$rowActive}", $row->invoice_no_2, PHPExcel_Cell_DataType::TYPE_STRING);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", $row->exchange_rate);
