...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN p.pph_amount
	...
	WHEN p.freight_id IS NOT NULL THEN (fc.freight_quantity * fc.freight_price) * (SELECT (a.pph/100) FROM freight a WHERE a.freight_id = p.freight_id)
	WHEN p.vendor_handling_id IS NOT NULL THEN (hc.handling_quantity * hc.handling_price) * (SELECT (a.pph/100) FROM vendor_handling a WHERE a.vendor_handling_id = p.vendor_handling_id)
	WHEN p.labor_id IS NOT NULL THEN uc.unloading_price * (SELECT a.pph/100 FROM labor a WHERE a.labor_id = uc.labor_id)
...
$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", "Total Amount");
$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", "Payment No");
$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", "Payment Date");
$objPHPExcel->getActiveSheet()->setCellValue("AB{$rowActive}", "Payment Status");
$objPHPExcel->getActiveSheet()->setCellValue("AC{$rowActive}", "Tarif");
$objPHPExcel->getActiveSheet()->setCellValue("AD{$rowActive}", "Nama");
$objPHPExcel->getActiveSheet()->setCellValue("AE{$rowActive}", "NPWP");
$objPHPExcel->getActiveSheet()->setCellValue("AF{$rowActive}", "NIK");
$objPHPExcel->getActiveSheet()->setCellValue("AG{$rowActive}", "Alamat");
$objPHPExcel->getActiveSheet()->setCellValue("AH{$rowActive}", "PKHOA");
...
while($row = mysqli_fetch_array($result)){
				...
				if($row['payment_no'] != '') {
				  ...
				  $dppNet = $row['dpp_pelunasan'] - $row['dp_dpp'];
	...
	$objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $row['dpp_pelunasan']);
	...
	$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", $row['dp_dpp']);
	...
	$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", $grand_total);
	$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", $voucherCode .' # '. $row['payment_no']);
	$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", $row['payment_date']);
	$objPHPExcel->getActiveSheet()->setCellValue("AB{$rowActive}", $row['p_status']);
	$objPHPExcel->getActiveSheet()->setCellValue("AC{$rowActive}", $row['tarif']);
	$objPHPExcel->getActiveSheet()->setCellValue("AD{$rowActive}", $row['nama']);
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AE{$rowActive}", $row['npwp'], PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AF{$rowActive}", $row['nik'], PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValue("AG{$rowActive}", $row['alamat']);
	//$objPHPExcel->getActiveSheet()->setCellValue("AH{$rowActive}", $row['doc_pkhoa']);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("AH{$rowActive}", $row['doc_pkhoa'], PHPExcel_Cell_DataType::TYPE_STRING);
...
// Set column width
for ($temp = ord("A"); $temp <= ord("AH"); $temp++) {
...
// Set number format for Amount 
$objPHPExcel->getActiveSheet()->getStyle("P" . ($headerRow + 1) . ":Y{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
