...
if($pksSourceId == 'CONTRACT'){
 ...
 $sql = "CALL SP_PKS_SourceCollection_Report('{$stockpileId}', '{$periodFrom}', '{$periodTo}')";
...
while($row = $result->fetch_object()) {
    ...
    if($Amount == 'PKS'){
        ...
        $AmountSusut = ($row->fc_shrink) + ($tempAmountSusut);
        $AmountInventory = ($row->amountFreight - $row->fc_shrink) + ($row->amountHandling) + ($row->Unloading) + ($tempAmountInventory);
