...
while($rowSP = $resultSP->fetch_object()) {
    ...
    $sql = " SELECT 
        ...
        FROM (SELECT {$whereProperty}, 
                    ...
                    --    CASE WHEN (lab.laborRules = '1') THEN SUM(tr.unloading_price)
                    --         WHEN (lab.laborRules = '2') THEN 
                    --             CASE WHEN (tr.send_weight < tr.netto_weight)
                    --                 THEN SUM(tr.send_weight * tr.unloading_price) 
                    --             ELSE SUM(tr.netto_weight * tr.unloading_price) 
                    --             END
                    --         WHEN (lab.laborRules = '3') THEN SUM(tr.netto_weight * tr.unloading_price)
                    --         WHEN (lab.laborRules = '4') THEN SUM(tr.send_weight * tr.unloading_price)
                    --     ELSE NULL 
                    --     END AS Unloading,
                        ...
                        {$whereProperty1}
                        GROUP BY tr.slip_no 
) AS a GROUP BY {$whereProperty2}, a.typePKS
...
for ($x = 0; $x <= $tempResult; $x++){
    ...
    if($value == 'PKS'){
        ...
        $tempAmountInventory = ($row->amountFreight) + ($row->amountHandling) + ($row->newUnloading) + $tempPriceInventory;
       
        $testSW = $testSW + $tempPriceSW;
        $testFr = $testFr + $row->amountFreight;
    }
    $array[] = $row;
	if($tipePKS == 'P'){
		$temp = 1;
