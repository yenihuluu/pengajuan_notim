...
$AmountTotalQty = $total_quantityR + $total_quantityG + $total_quantityRG + $totalCogs2UN;
$AmountTotalPKS = $total_pksR + $total_pksG + $total_pksRG + $total_pksUN ;
$AmountTotalFC = $total_fcR + $total_fcG + $total_fcRG + $total_fcUN;
$AmountTotalVH = $total_vhR + $total_vhG + $total_vhRB + $total_vhUN;
$AmountTotalUC = $total_ucR + $total_ucG + $total_ucRB + $total_ucUN;
...
$sql ="SELECT ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.rsb = 1 AND t.ggl = 0 THEN t.quantity ELSE 0 END)) AS qty_availableR, 
		ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.rsb = 0 AND t.ggl = 1 THEN t.quantity ELSE 0 END)) AS qty_availableG, 
		ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.rsb = 1 AND t.ggl = 1 THEN t.quantity ELSE 0 END)) AS qty_availableRG, 
		ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.rsb = 0 AND t.ggl = 0 THEN t.quantity ELSE 0 END)) AS qty_availableUN,
