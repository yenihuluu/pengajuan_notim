...
if($shipmentId != '') {
    ...
    $sql = "SELECT sh.*,s.stockpile_code FROM shipment sh INNER JOIN sales sl ON sh.sales_id = sl.sales_id INNER JOIN stockpile s ON sl.stockpile_id = s.stockpile_id  WHERE sh.sales_id = {$shipmentId}";
    ...
    $objPHPExcel->getActiveSheet()->mergeCells("C{$rowActive}:{$lastColumn}{$rowActive}");
			...
			if($row2->slip_no >= 'MAR-0000000001' && $row2->slip_no <= 'MAR-0000007138'){
				...
				if($row2->vh_pph_tax_category == 1 && $row2->vh_pph_tax_id != ''){
				 ...
				 $freightTotal2 = $row2->freight_total + $ppnfc2 + $pphfc2;
...
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "UNLOADING PRICE");
...
while($row = $result->fetch_object()) {
    $rowActive++;
      ...
      if($row->slip_no >= 'DUM-19-0000002529' && $row->slip_no <= 'DUM-19-0000002868' && $row->freight_code == 'FREDY-GPH'){
          $fp = ($row->percent_taken / 100) * ($row->send_weight * $row->freight_price);

      }else{
        $fp = ($row->percent_taken / 100) * ($row->quantity * $row->freight_price);
      }
