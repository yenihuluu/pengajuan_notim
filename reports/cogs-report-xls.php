...
$sql2 = "SELECT d.*,
			...
			CASE WHEN t.mutasi_id IS NOT NULL THEN t.unit_price
			WHEN t.adjustmentAudit_id IS NOT NULL THEN t.unit_price
			ELSE con.price_converted END AS price_converted,
            CASE WHEN t.mutasi_id IS NOT NULL THEN d.quantity * t.unit_price
	    ...
	    WHEN t.freight_cost_id IS NOT NULL THEN (d.percent_taken / 100) * COALESCE((SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = d.transaction_id),0) ELSE 0 END AS freight_shrink,
			...
			(SELECT transaction_date FROM TRANSACTION WHERE shipment_id = (SELECT shipment_id FROM shipment WHERE sales_id = sl.sales_id ORDER BY shipment_id ASC LIMIT 1)LIMIT 1) AS transactionDate,
			...
			(SELECT vehicle_no FROM transaction WHERE shipment_id = (SELECT shipment_id FROM shipment WHERE sales_id = sl.sales_id ORDER BY shipment_id ASC LIMIT 1)LIMIT 1) AS vessel_name, sh.shipment_no
                     FROM delivery d
...
$sql3 = "SELECT * FROM (
SELECT SUM(t2.quantity) AS qty, t2.slip_no FROM `transaction` t2 INNER JOIN shipment sh ON sh.shipment_id = t2.shipment_id WHERE 1=1
...
$sql = "SELECT d.*,
            ...
            v1.vendor_name, 
            sh.shipment_no,
            t.send_weight, t.netto_weight, d.quantity, CASE WHEN t.mutasi_id IS NOT NULL THEN t.unit_price
			WHEN t.adjustmentAudit_id IS NOT NULL THEN t.unit_price
			ELSE con.price_converted END AS price_converted,
            CASE WHEN t.mutasi_id IS NOT NULL THEN d.quantity * t.unit_price
			WHEN t.adjustmentAudit_id IS NOT NULL THEN d.quantity * t.unit_price
	    ...
	    WHEN t.freight_cost_id IS NOT NULL THEN (d.percent_taken / 100) * COALESCE((SELECT amt_claim FROM transaction_shrink_weight WHERE transaction_id = d.transaction_id),0) ELSE 0 END AS freight_shrink,
            ...
            l.ppn_tax_id AS uc_ppn_tax_id, l.ppn AS uc_ppn, uctxppn.tax_category AS uc_ppn_tax_category,
            l.pph_tax_id AS uc_pph_tax_id, l.pph AS uc_pph, uctxpph.tax_category AS uc_pph_tax_category,
			l.labor_id,t.freight_cost_id, f.freight_id,f.freight_supplier, l.labor_name, v1d.distance, v1d.ghg
                     FROM delivery d
        LEFT JOIN `transaction` t
        	ON t.transaction_id = d.transaction_id
        LEFT JOIN stockpile_contract sc
...
$objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", "Product (PKS)");

$objPHPExcel->getActiveSheet()->setCellValue("P{$rowMerge}", "Berat Kirim (kg)");
$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowMerge}", "Berat Netto (kg)");
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowMerge}", "Inventory (kg)");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowMerge}", "Berat Susut (kg)");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowMerge}", "Price /kg");
$objPHPExcel->getActiveSheet()->setCellValue("U{$rowMerge}", "COGS PKS Amount");

$objPHPExcel->getActiveSheet()->mergeCells("V{$rowActive}:V{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", "FREIGHT COST");
$objPHPExcel->getActiveSheet()->mergeCells("W{$rowActive}:W{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", "UNLOADING COST");
$objPHPExcel->getActiveSheet()->mergeCells("X{$rowActive}:X{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", "HANDLING COST");
$objPHPExcel->getActiveSheet()->mergeCells("Y{$rowActive}:Y{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", "TOTAL COGS");
...
while($row = $result->fetch_object()) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $row->shipment_no);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $row->netto_weight);
    $objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", $row->quantity);
    $objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "");
    $objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $row->price_converted);
    $objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", $row->cogs_amount);
    $objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", $freightTotal);
    $objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", $unloadingTotal);
	$objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", $handlingTotal);
    $objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", $totalCogs);
}
