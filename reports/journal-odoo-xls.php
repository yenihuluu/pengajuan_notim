...
$fileName = "Journal Report" . $period . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "Y";
...
while ($row = $result->fetch_object()) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", $row->account_no);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", number_format($row->quantity, 0, ".", ","));
    $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", number_format($row->price, 0, ".", ","));
    $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", number_format($row->debitAmount, 0, ".", ","));
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", number_format($row->creditAmount, 0, ".", ","));
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", number_format($original_amount, 0, ".", ","));
    $objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", $customCurrency);
    $objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", number_format($row->exchange_rate, 0, ".", ","));
    $no++;
