...
if(isset($_POST['periodFrom']) && $_POST['periodFrom'] != '' && isset($_POST['periodTo']) && $_POST['periodTo'] != '' && isset($_POST['requestDate']) && $_POST['requestDate'] != '') {
	...
	$whereR2 .= " OR ((pgeneral.requestDateGeneral = '{$requestDate}') OR (pengajuanOA.requestDateOA = '{$requestDate}') OR (purchasing.requestDatePKS = '{$requestDate}'))";
...
$sql1 = "SELECT DATE_FORMAT(logbook.entry_date, '%Y/%m/%d') AS tgl,
	    ...
	    pengajuanOA.requestDateOA,
            ...
            LEFT JOIN (
                ...
                IFNULL(SUM(pgd.tamount_converted),0) AS nilai_pGeneral,
            ...
            LEFT JOIN (
                SELECT DATE_FORMAT(pg.entry_date, '%Y/%m/%d') AS tgl,
                    ...
                    IFNULL(SUM(pgd.tamount_converted),0) AS Reject_nilaiGeneral
                    FROM pengajuan_general pg 
                    LEFT JOIN pengajuan_general_detail pgd ON pgd.pg_id = pg.pengajuan_general_id
            ...
            LEFT JOIN (	
                ...
                IFNULL(SUM(CASE WHEN pg.invoice_id IS NOT NULL AND p.payment_no IS NULL THEN invd.tamount_converted ELSE NULL END),0) AS nilai_processGeneral
...
//DATA LOGBOOK
$sql = "SELECT 
        CASE WHEN l.type_pengajuan = 1 THEN 'Curah/OA/OB/Handling' WHEN l.type_pengajuan = 2 THEN 'General' ELSE 'PKS KONTRAK' END AS typePengajuan, 
        CASE WHEN l.type_pengajuan = '1' THEN CASE WHEN pp.payment_method = '1' 
        OR l.type_pengajuan = '3' THEN 'Payment' WHEN pp.payment_method = '2' THEN 'Down Payment' ELSE NULL END WHEN l.type_pengajuan = '2' 
        OR l.type_pengajuan = '3' THEN 'payment' ELSE NULL END AS method, 
        CASE WHEN l.type_pengajuan = '1' THEN DATE_FORMAT(pp.entry_date, '%d/%m/%Y') WHEN l.type_pengajuan = '2' THEN DATE_FORMAT(d.tgl, '%d/%m/%Y') ELSE DATE_FORMAT(pur.entry_date, '%d/%m/%Y') END AS tanggal_pengajuan, 
        CASE WHEN l.type_pengajuan = '1' THEN DATE_FORMAT(pp.email_date, '%d/%m/%Y') WHEN l.type_pengajuan = '2' THEN DATE_FORMAT(
        d.pengajuan_email_date, '%d/%m/%Y'
        ) ELSE NULL END AS tglEmail, 
        sp.stockpile_name, 
        us.user_name AS PICPengajuan, 
        CASE WHEN l.type_pengajuan = '1' THEN CASE WHEN pp.urgent_payment_type = 1 THEN 'URGENT' ELSE 'NORMAL' END WHEN l.type_pengajuan = '2' THEN CASE WHEN d.payment_type = 1 THEN 'URGENT' ELSE 'NORMAL' END ELSE 'NORMAL' END AS urgentType, 
        CASE WHEN l.type_pengajuan = '1' THEN 
        CASE WHEN pp.vendor_id IS NOT NULL THEN v.vendor_name 
        ...
        CASE WHEN l.type_pengajuan = '1' THEN 
        CASE WHEN pp.vendor_id IS NOT NULL THEN vb.bank_name 
            WHEN pp.freight_id IS NOT NULL THEN fb.bank_name 
            WHEN pp.labor_id IS NOT NULL THEN lbb.bank_name 
            WHEN pp.vendor_handling_id IS NOT NULL THEN vhb.bank_name ELSE NULL END 
            WHEN l.type_pengajuan = '2' THEN d.bank_name 
            WHEN l.type_pengajuan = '3' THEN vbp.bank_name 
        ELSE NULL END AS bankVendor, 

        CASE WHEN l.type_pengajuan = '1' THEN pp.remarks WHEN l.type_pengajuan = '2' THEN d.remarks ELSE NULL END AS remarks, 
        CASE WHEN l.type_pengajuan = '1' THEN pp.dpp WHEN l.type_pengajuan = '2' THEN d.amount WHEN l.type_pengajuan = '3' THEN (pur.price * pur.quantity) ELSE NULL END AS dpp, 
        CASE WHEN l.type_pengajuan = '1' THEN pp.ppn_amount WHEN l.type_pengajuan = '2' THEN d.ppn1 WHEN l.type_pengajuan = '3' THEN CASE WHEN pur.ppn = 1 THEN 0 WHEN pur.ppn = 2 THEN (pur.price * pur.quantity) * (11 / 100) ELSE NULL END ELSE NULL END AS ppn, 
        CASE WHEN l.type_pengajuan = '1' THEN pp.pph_amount WHEN l.type_pengajuan = '2' THEN d.pph1 WHEN l.type_pengajuan = '3' THEN 0 ELSE NULL END AS pph, 
        CASE WHEN l.type_pengajuan = '1' THEN pp.amount WHEN l.type_pengajuan = '2' THEN (
        (d.amount + d.ppn1) - d.pph1
        ) WHEN l.type_pengajuan = '3' THEN CASE WHEN pur.ppn = 1 THEN (pur.price * pur.quantity) WHEN pur.ppn = 2 THEN (pur.price * pur.quantity) + (
        (pur.price * pur.quantity) * (11 / 100)
        ) ELSE NULL END ELSE NULL END AS amount, 
        CASE WHEN l.type_pengajuan = '1' THEN ino.inv_notim_no WHEN l.type_pengajuan = '2' THEN inv.invoice_no ELSE NULL END AS invoiceNo2, 
        p.payment_no, 
        CASE WHEN l.type_pengajuan = '1' THEN DATE_FORMAT(
        pp.urgent_payment_date, '%d-%m-%Y'
        ) WHEN l.type_pengajuan = '2' THEN DATE_FORMAT(
        d.request_payment_date, '%d-%m-%Y'
        ) WHEN l.type_pengajuan = '3' THEN DATE_FORMAT(
        pur.plan_payment_date, '%d-%m-%Y'
        ...
        FROM 
            USER 
        WHERE 
            user_id = buh.entry_by
        ) AS PICPaid, 
        bud.grand_total, 
        bud.batch_code, 
        CASE WHEN buh.entry_date IS NULL THEN CASE WHEN l.type_pengajuan = '1' THEN (
        SELECT 
            TIMESTAMPDIFF(
            DAY, 
            pp.entry_date, 
            STR_TO_DATE('05/09/2022', '%d/%m/%Y')
            )
        ...
        ) ELSE 0 END ELSE NULL END AS AgingPending, 
        CASE WHEN (
        pp.inv_notim_id IS NULL 
        AND bud.batch_code IS NULL 
        AND l.status1 != 2 
        AND pp.entry_date IS NOT NULL
        ) 
        OR (
        d.status_pengajuan = 0 && d.status_pengajuan != 5
        ) 
        OR (l.status1 = 0) THEN 'Pengajuan' WHEN (
        (
            l.payment_id IS NOT NULL 
            OR l.inv_notim_id IS NOT NULL 
            OR l.inv_general_id IS NOT NULL
        ) 
        AND bud.batch_code IS NULL 
        AND l.status1 != 2
        ) THEN 'On Process' WHEN bud.batch_code IS NOT NULL 
        AND l.payment_id IS NOT NULL 
        AND l.status1 != 2 THEN 'Paid' WHEN l.status1 = 2 THEN 'Cancel' WHEN l.status1 = 4 THEN 'Reject' ELSE NULL END AS statusLogbook 
        #and d.status_pengajuan = 5 F
        FROM logbook_new l 
        ...
        ( SELECT pg.entry_date AS tgl, 
            CASE WHEN pgd.termin < 100 THEN IFNULL(SUM(pgd.amount - (pgd.amount * (pgd.termin/100))),0) ELSE IFNULL(SUM(pgd.amount),0) END AS amount, 
            CASE WHEN pgd.termin < 100 THEN IFNULL(SUM(pgd.pph - (pgd.pph * (pgd.termin/100))),0) ELSE IFNULL(SUM(pgd.pph),0) END AS pph1, 
            CASE WHEN pgd.termin < 100 THEN IFNULL(SUM(pgd.ppn - (pgd.ppn * (pgd.termin/100))),0) ELSE IFNULL(SUM(pgd.ppn),0) END AS ppn1, 
            pg.`pengajuan_general_id`, pg.pengajuan_email_date, pg.stockpileId, pg.entry_by, pg.entry_date, gv.general_vendor_name, gvb.bank_name,
            pg.remarks, pg.status_pengajuan, pg.request_payment_date, 
            pg.payment_type FROM pengajuan_general pg 
            LEFT JOIN pengajuan_general_detail pgd ON pgd.pg_id = pg.pengajuan_general_id 
            LEFT JOIN general_vendor gv ON gv.general_vendor_id = pgd.general_vendor_id 
            LEFT JOIN general_vendor_bank gvb ON gvb.gv_bank_id = pgd.gv_bank_id
            GROUP BY pgd.pg_id 
        ...
        LEFT JOIN vendor v ON v.vendor_id = pp.vendor_id OR v.vendor_id = pur.vendor_id LEFT JOIN freight f ON f.freight_id = pp.freight_id 
        LEFT JOIN labor lb ON lb.labor_id = pp.labor_id LEFT JOIN vendor_handling vh ON vh.vendor_handling_id = pp.vendor_handling_id 
        LEFT JOIN invoice inv ON inv.invoice_id = l.inv_general_id LEFT JOIN invoice_notim ino ON ino.inv_notim_id = l.inv_notim_id 
        LEFT JOIN payment p ON p.payment_id = l.payment_id LEFT JOIN batch_upload_detail bud ON bud.payment_id = l.payment_id 
        LEFT JOIN batch_upload_header buh ON buh.batch_code = bud.batch_code 
        LEFT JOIN vendor_bank vb ON vb.v_bank_id = pp.vendor_bank_id
        LEFT JOIN freight_bank fb ON fb.f_bank_id = pp.vendor_bank_id
        LEFT JOIN labor_bank lbb ON lbb.l_bank_id = pp.vendor_bank_id
        LEFT JOIN vendor_handling_bank vhb ON vhb.vh_bank_id = pp.vendor_bank_id
		LEFT JOIN vendor_bank vbp ON vbp.v_bank_id = p.vendor_bank_id
...
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Bank Vendor");
...
$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", "Total Amount");

$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "No. Invoice");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "No. PV");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", "PIC Finance");
$objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", "Remaks E-Banking");
$objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", "Tanggal Pembayaran");
$objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", "Jumlah Pembayaran");
$objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", "PIC Pembayaran");
$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", "Batch");
$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", "Aging");
$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", "Catatan");
...
while($row = $resultData->fetch_object()) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->bankVendor);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $row->amount);
    
    $objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", $row->invoiceNo2);
    $objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", $row->payment_no);
    $objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $row->PICFinance);
    $objPHPExcel->getActiveSheet()->setCellValue("U{$rowActive}", $row->kode3);
    $objPHPExcel->getActiveSheet()->setCellValue("V{$rowActive}", $row->batchDate);
    $objPHPExcel->getActiveSheet()->setCellValue("W{$rowActive}", $row->grand_total);
    $objPHPExcel->getActiveSheet()->setCellValue("X{$rowActive}", $row->PICPaid);
	$objPHPExcel->getActiveSheet()->setCellValue("Y{$rowActive}", $row->batch_code);
	$objPHPExcel->getActiveSheet()->setCellValue("Z{$rowActive}", $row->AgingPending);
	$objPHPExcel->getActiveSheet()->setCellValue("AA{$rowActive}", "Remaks");
		$objPHPExcel->getActiveSheet()->setCellValue("BB{$rowActive}", $row->batch_code);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $GrandTotalPengajuan);
    $objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $GrandTotalPembayaran);
...
for ($temp = ord("B"); $temp <= ord("Z"); $temp++) {
    ...
    $objPHPExcel->getActiveSheet()->getStyle("M" . ($headerRow1 + 1) . ":P{$bodyRowEnd1}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1); // Set number format for Amount 
