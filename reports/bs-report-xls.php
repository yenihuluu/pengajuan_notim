...
$fileName = "BS Report " . $period . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "O";
...
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "BS Report");
...
while($row = $result->fetch_object()) {
  ...
  $rowActive++;
   ...
   //$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", $row->stockpile_name);
    $objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", $row->account_no);
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("C{$rowActive}", $row->account_name, PHPExcel_Cell_DataType::TYPE_STRING);
...
$objPHPExcel->getActiveSheet()->getStyle("D" . ($headerRow + 1) . ":F{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
