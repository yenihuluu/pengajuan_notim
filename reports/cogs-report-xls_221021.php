...
FROM `transaction` t 
WHERE 1=1 AND t.slip_no < '{$slipOut}' AND t.transaction_date <= '{$transactionDate}' AND SUBSTRING(t.slip_no,1,3) = '{$slipOutCode}' ORDER by t.transaction_date ASC";
...
while($row = $result->fetch_object()) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $distance);
	$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $ghg);
