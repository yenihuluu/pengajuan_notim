...
$objPHPExcel->getActiveSheet()->mergeCells("G{$rowActive}:G{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", "SHIPMENT CODE");
$objPHPExcel->getActiveSheet()->mergeCells("H{$rowActive}:H{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "INVENTORY (KG)");
$objPHPExcel->getActiveSheet()->mergeCells("I{$rowActive}:I{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "PKS SOURCE DETAIL");

$objPHPExcel->getActiveSheet()->mergeCells("J{$rowActive}:J{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", "TANGGAL MASUK");
$objPHPExcel->getActiveSheet()->mergeCells("K{$rowActive}:K{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", "TANGGAL MUAT");
$objPHPExcel->getActiveSheet()->mergeCells("L{$rowActive}:L{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "KODE MASUK MOBIL");
$objPHPExcel->getActiveSheet()->mergeCells("M{$rowActive}:M{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "KODE TERIMA");
$objPHPExcel->getActiveSheet()->mergeCells("N{$rowActive}:N{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "TIKET TIMBANG");
$objPHPExcel->getActiveSheet()->mergeCells("O{$rowActive}:O{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "CONTRACT NO");
...
while($row = $result->fetch_object()) {
   ...
   /* $objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $row->freight_code);
    ...
    $objPHPExcel->getActiveSheet()->setCellValueExplicit("G{$rowActive}", $row->shipment_no, PHPExcel_Cell_DataType::TYPE_STRING);
	...
	$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $row->vendor_curah_name);
	
	$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $row->transaction_date2);
	$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $row->tglMuat);
    $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->kodeMasukMobil);
	
	$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", $row->KodeTerima);
	 $objPHPExcel->getActiveSheet()->setCellValueExplicit("N{$rowActive}", $row->tiketTimbang, PHPExcel_Cell_DataType::TYPE_STRING);
     $objPHPExcel->getActiveSheet()->setCellValueExplicit("O{$rowActive}", $row->contract_no, PHPExcel_Cell_DataType::TYPE_STRING);
