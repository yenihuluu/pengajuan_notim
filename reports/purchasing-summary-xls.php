...
if ($periodFrom != '' && $periodTo != '') {
    ...
    $whereProperty2 .= " AND c.entry_date BETWEEN STR_TO_DATE('{$periodFrom}', '%d/%m/%Y') AND STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
    $periodFull = $periodFrom . " - " . $periodTo . " ";
} else if ($periodFrom != '' && $periodTo == '') {

    $whereProperty2 .= " AND c.entry_date >= STR_TO_DATE('{$periodFrom}', '%d/%m/%Y')";
    $periodFull = "From " . $periodFrom . " ";
}else if ($periodFrom == '' && $periodTo != '') {

    $whereProperty2 .= " AND c.entry_date <= STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
...
$sql = "SELECT sc.stockpile_contract_id, i.invoice_no,
...
(SELECT GROUP_CONCAT(gv.general_vendor_name) FROM general_vendor gv WHERE gv.general_vendor_id = id.general_vendor_id AND a.account_no IN (520900,521000) AND i.`invoice_status` = 0) AS vendor_fee,
(IFNULL((SELECT SUM(id.amount_converted) FROM invoice_detail id WHERE id.poId = sc.contract_id AND i.`invoice_status` = 0 AND a.account_no IN (520900,521000)) / sc.`quantity`,0)) AS fee_price,
...
if ($periodFrom != "" && $periodTo != "") {
	...
	$rowActive++;
    $objPHPExcel->getActiveSheet()->mergeCells("A{$rowActive}:{$lastColumn}{$rowActive}");
    $objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "Period >= {$periodFrom}");
} else if ($periodFrom == "" && $periodTo != ""){
	$rowActive++;
    $objPHPExcel->getActiveSheet()->mergeCells("A{$rowActive}:{$lastColumn}{$rowActive}");
    $objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "Period <= {$periodTo}");
}
