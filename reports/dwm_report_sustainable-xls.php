...
if(isset($_POST['periodFrom']) && $_POST['periodFrom'] != '' && isset($_POST['periodTo']) && $_POST['periodTo'] != '') { //periode from, periode to
    ...
    if($baseOn == 'monthly'){ //Monthly
       ...
       $whereProperty .= "MONTHNAME(transaction_date) AS tanggal1, DATE_FORMAT(transaction_date, '%Y-%m') as tanggal";
...
$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", "Total Collection");
$objPHPExcel->getActiveSheet()->mergeCells("K{$rowActive}:K{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", "Outgoing Data");
$objPHPExcel->getActiveSheet()->mergeCells("L{$rowActive}:L{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Stock");
$objPHPExcel->getActiveSheet()->mergeCells("M{$rowActive}:M{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "Status Claim");
...
$objPHPExcel->getActiveSheet()->mergeCells("G{$rowActive}:I{$rowActive}");
$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", "Incoming from PKS Curah");
...
$objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", "Susut");
$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", "Inventory Weight");
...
$sql = " SELECT 
            COALESCE(notimOut.inventoryOut,0) AS notimOut1, 
            COALESCE(stock.stock,0) AS stock1, 
            SUM(COALESCE(a.Unloading1,0)) AS newUnloading, 
            SUM(COALESCE(a.AmountSendW1,0)) AS AmountSendW,
            SUM(COALESCE(a.AmountSusut1,0)) AS AmountSusut,
            SUM(COALESCE(a.AmountInventory1,0)) AS AmountInventory,
            ...
            a.* FROM(SELECT {$whereProperty}, 
                ...
                IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$rowSP->stockpile_id})  
                {$whereProperty1}
                AND con.langsir = 0 AND tr.transaction_type = 1
                GROUP BY tr.slip_no 
            ) AS a 
            LEFT JOIN (
                SELECT {$whereProperty},
                tr.stockpile_contract_id,
                SUM(COALESCE(tr.inventory_value,0)) AS inventoryOut,
                con.contract_type
                FROM TRANSACTION tr
                LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = tr.stockpile_contract_id
                LEFT JOIN stockpile sp ON sp.stockpile_id = sc.stockpile_id 
                LEFT JOIN contract con ON con.contract_id = sc.`contract_id`
                WHERE tr.transaction_type = 2 AND  SUBSTRING(tr.slip_no,1,3) IN (SELECT stockpile_code FROM stockpile WHERE stockpile_id = {$rowSP->stockpile_id})
                {$whereProperty1}
                GROUP BY tanggal
            ) AS notimOut ON notimOut.tanggal = a.tanggal
            LEFT JOIN (
                SELECT {$whereProperty},
                ROUND(SUM(CASE WHEN tr.transaction_type = 1 THEN tr.quantity ELSE -1 * tr.`quantity` END) -
                SUM(CASE WHEN tr.transaction_type = 2 THEN tr.shrink ELSE 0 END),2) AS stock
                FROM TRANSACTION tr
                LEFT JOIN stockpile_contract sc ON sc.stockpile_contract_id = tr.stockpile_contract_id
                LEFT JOIN stockpile sp ON sp.stockpile_id = sc.stockpile_id 
                LEFT JOIN contract con ON con.contract_id = sc.`contract_id`
                WHERE tr.transaction_type = 1 
                AND  tr.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$rowSP->stockpile_id})  
                {$whereProperty1} 
                GROUP BY tanggal
            ) AS stock ON stock.tanggal = a.tanggal
GROUP BY {$whereProperty2}, typePKS
ORDER BY {$whereProperty3}, typePKS DESC";
$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
// AND con.langsir = 0
// echo $sql;
...
// $testFr = 0;
        ...
        for ($x = 1; $x <= $tempResult; $x++){
            ...
            if($tipePKS == 'P'){
                ...
                if($baseOn == 'weekly'){
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", $row->tanggal1);
                }else{
            ...
            if($tipePKS == 'P'){
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $row->susut);
                $objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $row->inventory);
                ...
                $totalOutgoing = $totalOutgoing + $row->notimOut1;
                $totalStock2 = $totalStock2 + $row->stock1;
                ...
                $totalNotimOut = $row->notimOut1;
                $totalStock = $row->stock1;

                $objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $totalQtyCollection);
                $objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $totalNotimOut);
                $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $totalStock);
                $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", '0');
    ...
    // $rowActive++;
        $objPHPExcel->getActiveSheet()->mergeCells("B{$rowActive}:J{$rowActive}");
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $TempSumQtySusut);
    $objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $TempSumQtyInv);

    $objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", $TempSumQtySW_curah);
    $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $TempSumQtySusut_curah);
    $objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $TempSumQtyInv_curah);

    $objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $GrandtotalQTY);
    $objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $totalOutgoing);
    $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $totalStock2);
    $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", 0);
   ...
   //die();
    ...
    if($baseOn == 'monthly'){
        ...
        $objPHPExcel->getActiveSheet()->getStyle("D" . ($headerRow + 1) . ":M{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1); // Set number format for Amount 
