...
if (isset($_POST['stockpile']) && $_POST['stockpile'] != '') {
    ...
    $sql .= " AND stockpile = '{$stockpile}'";
}


$sql .= " AND regenerate = 0 ORDER BY jurnal_no ASC";
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#btnSend').click(function (e) {
            ...
            // $('#dataSearch').load('searchs/journal-odoo.php');
            $('#dataContent').load('./contoh_jatim.php', $("#sendForm").serialize(), iAmACallbackFunction2);
        });
...
<div class="row-fluid">
    ...
    <form id="sendForm" method="post" action="reports/journal-odoo-xls.php">
        <button class="btn btn-success">Download XLS</button>
        <input type="hidden" name="periodFrom" value="<?php echo $periodFrom; ?>"/>
        <input type="hidden" name="periodTo" value="<?php echo $periodTo; ?>"/>
        <input type="hidden" name="glDateFrom" value="<?php echo $glDateFrom; ?>"/>
        <input type="hidden" name="glDateTo" value="<?php echo $glDateTo; ?>"/>
        <input type="hidden" name="module" value="<?php echo $module; ?>"/>
		<input type="hidden" name="stockpile" value="<?php echo $stockpile; ?>"/>
        <table class="table table-bordered table-striped" id="contentTable" style="font-size: 8pt;">
            ...
            <?php
            if ($result === false) {
                echo '<tr><td colspan="26">Wrong Query</td></tr>';
                echo $sql;
            } else {
