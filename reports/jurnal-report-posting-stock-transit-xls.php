...
//MUTASI
$sqlMutasi = "SELECT mh.*,
				(SELECT stockpile_name FROM stockpile spFrom WHERE spFrom.stockpile_id = mh.stockpile_from) AS spFrom,
				(SELECT stockpile_name FROM stockpile spTo WHERE spTo.stockpile_id = mh.stockpile_to) AS spTo
			FROM mutasi_header mh where mh.mutasi_header_id = {$kodeMutasi}";
...
if ($resultMutasi !== false && $resultMutasi->num_rows > 0) {
    while ($rowMutasi = $resultMutasi->fetch_object()) {
		...
		$stockpileFrom = $rowMutasi->spFrom;
		$stockpileTo = $rowMutasi->spTo;
    }
}
...
WHERE  gl.amount > 0 AND general_ledger_module = 'INVOICE DETAIL' AND i.invoice_id IN ($invoiceId)";
$resultInvoice = $myDatabase->query($sql1, MYSQLI_STORE_RESULT);

//PAYMENT
$sql2 = "SELECT gl.*, b.bank_type, p.payment_type, inv.`invoice_no` AS invoiceNo, p.payment_no, date_format(p.payment_date, '%Y-%m-%d') as payment_date,
(SELECT `account_no` FROM account a WHERE a.account_id = gl.account_id) AS account_no, 
(SELECT `account_name` FROM account a WHERE a.account_id = gl.account_id) AS account_name 
FROM general_ledger gl 
...
//TRANSACTION NOTIM
$sql3 = "SELECT gl.*, DATE_FORMAT(t.unloading_date, '%Y-%m-%d') AS unloading_date, t.slip_no, t.unit_cost, t.quantity,
(SELECT `account_no` FROM  account a WHERE a.account_id = gl.account_id) AS account_no,
(SELECT `account_name` FROM  account a WHERE a.account_id = gl.account_id) AS account_name
...
$rowActive++;
$objPHPExcel->getActiveSheet()->getStyle("B{$rowActive}:{$lastColumn}{$rowActive}")->applyFromArray($styleArray3);
$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", "Stockpile From = {$stockpileFrom} ");

$rowActive++;
$objPHPExcel->getActiveSheet()->getStyle("B{$rowActive}:{$lastColumn}{$rowActive}")->applyFromArray($styleArray3);
$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", "Stockpile To = {$stockpileTo} ");
...
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Debit");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "Credit");
...
while ($rowContract = $resultContract->fetch_object()) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $debitAmount);
    $creditAmount = 0;
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $creditAmount);
    $no++;
}
...
$objPHPExcel->getActiveSheet()->getStyle("B{$rowActive}:{$lastColumn}{$rowActive}")->applyFromArray($styleArray2);
$objPHPExcel->getActiveSheet()->getRowDimension("{$rowActive}")->setRowHeight(20);
$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", "INVOICE DETAIL");
...
$objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", "Invoice");
$objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", "Account No");
$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", "Account Name");
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Debit");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "Credit");
...
while ($rowPayment = $resultPayment->fetch_object()) {
	...
	$objPHPExcel->getActiveSheet()->getCell("D{$rowActive}")->setValueExplicit($rowPayment->invoiceNo, PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->getCell("E{$rowActive}")->setValueExplicit($rowPayment->account_no, PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->getActiveSheet()->getCell("F{$rowActive}")->setValueExplicit($rowPayment->account_name, PHPExcel_Cell_DataType::TYPE_STRING);
    $debitAmount = 0;
...
$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", "Unit Cost");
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Debit");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "Credit");
$objPHPExcel->getActiveSheet()->getStyle("A{$rowActive}:{$lastColumn}{$rowActive}")->applyFromArray($styleArray8);
...
while ($rowTransaction = $resultTransaction->fetch_object()) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $rowTransaction->quantity);
	$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", $rowTransaction->unit_cost);
   $debitAmount = 0;
