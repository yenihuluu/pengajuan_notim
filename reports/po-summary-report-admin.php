...
$sql = "INSERT INTO user_access (user_id,access,access_date) VALUES ({$_SESSION['userId']},'VIEW PO SUMMARY REPORT',STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'))";
...
ELSE(SELECT p.payment_no FROM payment p LEFT JOIN stockpile_contract sc ON sc.`stockpile_contract_id` = p.`stockpile_contract_id` WHERE sc.`contract_id` = c.contract_id  AND p.payment_status = 0 ORDER BY sc.stockpile_contract_id ASC LIMIT 1) END AS payment_no,
...
ELSE (SELECT MAX(p.payment_date) FROM payment p LEFT JOIN stockpile_contract sc ON sc.`stockpile_contract_id` = p.`stockpile_contract_id` WHERE sc.`contract_id` = c.contract_id ORDER BY sc.stockpile_contract_id ASC LIMIT 1) END AS payment_date,
...
IFNULL((SELECT ROUND(SUM(t.send_weight),2) FROM `transaction` t LEFT JOIN stockpile_contract sc ON t.stockpile_contract_id = sc.stockpile_contract_id WHERE sc.contract_id = c.`contract_id` AND t.slip_no LIKE 'TYN%' {$whereProperty1} ),0) +
IFNULL((SELECT ROUND(SUM(t.send_weight),2) FROM `transaction` t LEFT JOIN stockpile_contract sc ON t.stockpile_contract_id = sc.stockpile_contract_id WHERE sc.contract_id = c.`contract_id` AND t.slip_no LIKE 'HO%' {$whereProperty1} ),0) +
(SELECT COALESCE(SUM(adjustment),0) FROM contract WHERE contract_id = c.contract_id {$whereProperty2})) > 0 THEN 'OPEN'
WHEN c.po_no = 'P-BSS/JPJ 001' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 001' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 002' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 003' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 004' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 005' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 006' THEN 'CLOSED'
WHEN c.po_no = 'P-BSS/JPJ 007' THEN 'CLOSED'
WHEN c.po_no = 'P-MIL BD Jaya 002' THEN 'CLOSED'
WHEN c.po_no = 'P-KSM/CPM 004' THEN 'CLOSED'
WHEN c.po_no = 'P-KSM/CPM 005' THEN 'CLOSED'
WHEN c.po_no = 'P-KSM/Penyangga 002' THEN 'CLOSED'
WHEN c.po_no = 'P-KSM/SAREH 002' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI2' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI3' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI4' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI(add1)' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI(add2)' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI (AHZ)' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI2 (AHZ) ' THEN 'CLOSED'
WHEN c.po_no = 'P-PBI3 (AHZ)' THEN 'CLOSED' ELSE 'OUTSTANDING' END AS `status`,
...
FROM contract c WHERE c.contract_type = 'P' AND c.contract_status != 2 {$whereProperty}
