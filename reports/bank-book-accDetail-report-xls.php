...
$fileName = "ListPaymentTransactionDetail" . $bankName . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "X";
...
if($bank_type == 1){
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "List Payment Transaction Detail Report");
}else{
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "List Payment Transaction Detail Report");	
}
$rowActive++;
...
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Harga /Kg");
...
WHERE 1=1 AND p.payment_id = '{$row->payment_id}'";
                    ...
                    $counter = 1;
					$rowMerge = $rowActive + $totalRow - 1;
					//$rowMerge2 = $rowMerge + $totalRow;
	...
	$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $row->stockpile_name2);
