...
if($vendorId != '') {
    $whereProperty1 .= " AND con.vendor_id IN (SELECT vendor_id FROM vendor WHERE vendor_name = '{$vendorId}') ";
    
    $sql = "SELECT * FROM vendor WHERE vendor_id IN (SELECT vendor_id FROM vendor WHERE vendor_name = '{$vendorId}')";
...
$sql = "SELECT con.po_no, p.payment_no, DATE_FORMAT(p.payment_date, '%d %b %Y') AS payment_date, con.contract_no, v.vendor_name, con.price_converted, con.quantity, 
            ...
            t.send_weight AS quantity_received, t.send_weight * con.price_converted AS amount_received,con.adjustment
