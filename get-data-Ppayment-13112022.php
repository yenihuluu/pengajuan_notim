...
switch ($_POST['action']) {
    ...
    break;
    case "setSlipSales":
        setSlipSales($_POST['stockpileIdSales'], $_POST['customerId'], $_POST['contractSales'], $_POST['checkedSlips'], $_POST['checkedSlipsDP'],$_POST['paymentFromSales'], $_POST['paymentToSales'], $_POST['idPP']);
        break;
...
function setSlipSales($stockpileIdSales, $customerId, $contractSales, $checkedSlips, $checkedSlipsDP, $paymentFromSales, $paymentToSales, $idPP) {
    global $myDatabase;
    $returnValue = '';
    $return_val = '';
    $whereProperty = '';
...
pp.`total_dpp`,pp.`total_amount`, pp.`total_ppn_amount`, pp.`total_pph_amount`, pp.`grand_total`, pp.`total_qty`
...
//            }
            ...
            if($checkedSlips != '') {
                ...
                $returnValue .= '<td><input type="hidden" name="checkedSlips[]" value="' . $row->transaction_id . '" onclick="checkSlipSales(' . $stockpileIdSales . ',' . $row->customer_id . ','. "'". $contractSales2 ."'".', \'NONE\', \'NONE\', \'' . $paymentFromSales . '\', \'' . $paymentToSales . '\');" /></td>';
...
}function setSlipFreightSales($stockpileId_1, $freightId_1, $contractFreight, $checkedSlips, $ppn, $pph, $paymentFromFP, $paymentToFP, $idPP)
{
    ...
    pp.total_amount, pp.total_ppn_amount, pp.total_pph_amount, pp.grand_total, pp.`stockpile_location`, f.freight_rule, txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category,txpph.tax_value AS pph_tax_value, sh.`shipment_no`
    FROM TRANSACTION t 
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
				...
				$returnValue .= '<td style="width: 10%;">' . $row->sim . '</td>';
                ...
                $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format($row->freight_price * $fq, 2, ".", ",") . '</td>';
                $returnValue .= '</tr>';
            }
        ...
        // $totalPpn = ($ppnValue/100) * $totalPrice;
                    ...
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPn" id="totalPPn" value="' . number_format($totalPpn, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId_1 . ', ' . $freightId_1 . ',' . "'" . $contractIds . "'" . ', this, document.getElementById(\'pph\'), \'' . $paymentFromFP . '\', \'' . $paymentToFP . '\');" /></td>';
                    $returnValue .= '</tr>';
        //$totalPph = ($pphValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right;">PPh '.$pphtext.'</td>';
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPh" id="totalPPh" value="' . number_format($totalPph, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId_1 . ', ' . $freightId_1 . ',' . "'" . $contractIds . "'" . ', document.getElementById(\'ppn\'), this, \'' . $paymentFromFP . '\', \'' . $paymentToFP . '\');" /></td>';
                    $returnValue .= '</tr>';
...
function setSlipCurah($stockpileId, $vendorId, $checkedSlips, $paymentFrom1, $paymentTo1, $idPP)
{
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            $dppTotalPrice = 0;
            if ($row->pph_tax_id == 0) {
                $dppTotalPrice = round(($row->unit_price * $row->quantity2), 2); //ceil di hapus
            } else {
                if ($row->pph_tax_category == 1) {
                    $dppTotalPrice = round(($row->unit_price * $row->quantity2),2) / round((100 - $row->pph_tax_value) / 100, 2); //ceil dihapus
                } else {
                    $dppTotalPrice = round(($row->unit_price * $row->quantity2), 2); //ceil dihapus
                }
            }
            ...
            $returnValue .= '<td style="text-align: right; width: 3%;">' . number_format($row->quantity2, 0, ".", ",") . '</td>';
            $returnValue .= '<td style="width: 2%;">IDR</td>';
        ...
        if ($boolcontinue) {
            ...
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="ppnCur" id="ppnCur" value="' . number_format($totalPPN, 0, ".", ",") . '" onblur="checkSlipCurah(' . $stockpileId . ',' . $vendorId . ', this, document.getElementById(\'pph\'), \'' . $paymentFrom1 . '\', \'' . $paymentTo1 . '\');" /></td>';
            $returnValue .= '</tr>';
         // $totalPph = ($pph/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="9">PPh</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="pphCur" id="pphCur" value="' . number_format($totalPPh, 0, ".", ",") . '" onblur="checkSlipCurah(' . $stockpileId . ',' . $vendorId . ', document.getElementById(\'ppn\'), this, \'' . $paymentFrom1 . '\', \'' . $paymentTo1 . '\');" /></td>';
            $returnValue .= '</tr>';
...
if($idPP == ''){
    $whereProperty = " AND (t.fc_ppayment_id IS NULL)";
}else if($idPP != ''){
    $whereProperty = " AND (t.fc_ppayment_id = {$idPP})";
  ...
  //echo $sql;
    ...
    if ($result->num_rows > 0) {
                ...
                if($idPP == ''){
                    if ($checkedSlips != '') {
                        ...
                        if ($pos === false) {
                            ...
                            $totalShrink2 = $totalShrink2 + $dppShrinkPrice;
                            $totalAddShrink = $totalAddShrink + $hsw_amt_claim;
                            $totalShrink = $totalShrink + ($dppShrinkPrice + $hsw_amt_claim);
							$tempTotalPrice = $tempTotalPrice + $amountPrice;
        ...
        // $totalPph = ($pphValue/100) * $totalPrice;
					...
					$totalPrice = $totalPrice - $totalShrink;
            ...
            $returnValue .= '</form>';
            $returnValue .= '<input type="hidden" id="totalPrice" name="totalPrice" value="' . round($totalPrice, 2) . '" />';
            $returnValue .= '<input type="hidden" id="totalPrice1" name="totalPrice1" value="' . round($tempTotalPrice, 2) . '" />';
            ...
            $returnValue .= '<input type="hidden" id="totalShrink2" name="totalShrink2" value="' . $totalShrink2 . '" />';
            $returnValue .= '<input type="hidden" id="totalAddShrink" name="totalAddShrink" value="' . $totalAddShrink . '" />';
...
function setSlipFreight_copy($stockpileId, $freightId, $vendorFreightId, $contractPKHOA, $checkedSlips, $ppn, $pph, $paymentFrom, $paymentTo, $idPP)
{
    ...
    if ($vendorFreightIds != 0) {
        $whereProperty = "AND t.vendor_id IN ({$vendorFreightIds})";
				...
				$sql = "SELECT fc.`contract_pkhoa`,COALESCE(hsw.amt_claim) AS hsw_amt_claim, COALESCE(ts.amt_claim,0) AS shrinkAmount,  t.*, sc.stockpile_id, fc.freight_id, con.po_no, f.freight_rule,
                f.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value, v.vendor_code,
                ts.`trx_shrink_claim`,
                ...
                ROUND((t.`freight_price` * t.`freight_quantity`) * (f.pph/100),10)  AS pph,
                ((ROUND(t.`freight_price` * t.`freight_quantity`,10) + ROUND((t.`freight_price` * t.`freight_quantity`) * (f.ppn/100),10) - ROUND((t.`freight_price` * t.`freight_quantity`) * (f.pph/100),10))) AS amount,
                (ROUND(t.`freight_price` * t.`freight_quantity`,10)  - (COALESCE(ts.amt_claim,0) + COALESCE(hsw.amt_claim,0) )) AS total_amount,
    ...
    if ($result->num_rows > 0) {
        ...
        //    }
            ...
            $returnValue .= '<tr>';
				...
				//	$totalPPh = ($totalPrice * ($row->pph_tax_value / 100));
					//$totalPph = $totalPph + $row->pph;
					$totalSusut = $totalSusut + ($row->shrinkAmount + $row->hsw_amt_claim);
            ...
            $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->qtyClaim, 0, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->trx_shrink_claim, 0, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format($row->shrinkAmount, 2, ".", ",") . '</td>';
    ...
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
        ...
        $sql = "SELECT t.*, sc.stockpile_id, vhc.vendor_handling_id, con.po_no, vh.vendor_handling_rule,
                ...
                AND t.transaction_date BETWEEN STR_TO_DATE('$paymentFromHP', '%d/%m/%Y') AND STR_TO_DATE('$paymentToHP', '%d/%m/%Y')
                 AND (t.posting_status = 2 OR (t.posting_status IS NULL OR t.posting_status = 0)) and t.hc_ppayment_id = {$idPP}
                ORDER BY t.slip_no ASC";
               // echo $sql;
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function setSlipHandling($stockpileId, $vendorHandlingId, $checkedSlips, $ppn, $pph, $paymentFromHP, $paymentToHP, $idPP)
{
    ...
    if ($result->num_rows > 0) {
        ...
        //    $totalPpn = ($ppnValue/100) * $totalPrice;
            ...
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="ppnH" id="ppnH" value="' . number_format($totalPpn, 2, ".", ",") . '" onblur="checkSlipFreight(' . $stockpileId . ', ' . $vendorHandlingId . ', this, document.getElementById(\'pph\'), \'' . $paymentFromHP . '\', \'' . $paymentToHP . '\');" /></td>';
            $returnValue .= '</tr>';
        //$totalPph = ($pphValue/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="9" style="text-align: right;">PPh</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="pphH" id="pphH" value="' . number_format($totalPph, 2, ".", ",") . '" onblur="checkSlipFreight(' . $stockpileId . ', ' . $vendorHandlingId . ', document.getElementById(\'ppn\'), this, \'' . $paymentFromHP . '\', \'' . $paymentToHP . '\');" /></td>';
            $returnValue .= '</tr>';
...
function setSlipUnloading($stockpileId, $laborId, $checkedSlips, $ppn, $pph, $paymentFromUP, $paymentToUP, $idPP)
{
    ...
    if ($result->num_rows > 0) {
        ...
        //    $totalPpn = ($ppn/100) * $totalPrice;
            ...
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="ppnOB" id="ppnOB" value="' . number_format($totalPpn, 2, ".", ",") . '" onblur="checkSlipUnloading(' . $stockpileId . ', ' . $laborId . ', this, document.getElementById(\'ppn\', \'' . $paymentFromUP . '\', \'' . $paymentToUP . '\');" /></td>';
            $returnValue .= '</tr>';
        //    $totalPph = ($pph/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="7" style="text-align: right;">PPh</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="pphOB" id="pphOB" value="' . number_format($totalPph, 2, ".", ",") . '" onblur="checkSlipUnloading(' . $stockpileId . ', ' . $laborId . ', document.getElementById(\'pph\'), this, \'' . $paymentFromUP . '\', \'' . $paymentToUP . '\');" /></td>';
            $returnValue .= '</tr>';
        ...
        $returnValue .= '<input type="hidden" id="qty" name="qty" value="' . $quanty . '" />';
        $returnValue .= '<input type="text" id="priceLabor" name="priceLabor" value="' . $price . '" />'; 
        $returnValue .= '<input type="hidden" id="dpp" name="dpp" value="' . $dpp . '" />'; 
        $return_val .= '<input type="hidden" value="|' . number_format($grandTotal, 2, ".", ",") . '|" />';  
        $returnValue .= '</div>';
    }
...
function setSlipPengajuanCurah_copy($stockpileId, $vendorId, $checkedSlips, $paymentFrom1, $paymentTo1, $idPP)
{
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
            $dppTotalPrice = 0;
            if ($row->pph_tax_id == 0) {
                $dppTotalPrice = round(($row->unit_price * $row->quantity2),2);
            } else {
                if ($row->pph_tax_category == 1) {
                    $dppTotalPrice = round(($row->unit_price * $row->quantity2),2) / round((100 - $row->pph_tax_value) / 100, 2);
                } else {
                    $dppTotalPrice = round(($row->unit_price * $row->quantity2),2);
                }
            }
        ...
        //    }
            ...
            $returnValue .= '<td style="width: 2%;">IDR</td>';
            $returnValue .= '<td style="text-align: right; width: 5%;">' . number_format($row->unit_price, 0, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format(($row->unit_price * $row->quantity2), 0, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 25%;">' . number_format($dppTotalPrice, 0, ".", ",") . '</td>';
            $returnValue .= '</tr>';
        }
